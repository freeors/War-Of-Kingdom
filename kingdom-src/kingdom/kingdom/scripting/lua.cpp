/* $Id: lua.cpp 52634 2012-01-17 14:57:24Z anonymissimus $ */
/*
...
static int impl_unit_get(lua_State *L)
{
	...
	return_cfg_attrib("__cfg", u.write(cfg); cfg["name"] = u.name(); u.get_location().write(cfg));
	return 0;
}
...
static int intf_get_units(lua_State *L)
{
	...
	if (!filter["cityno"].blank()) {
		// reside troop
		artifical* city = units.city_from_cityno(filter["cityno"].to_int());
		if (!city) {
			return 1;
		}
...
static int intf_simulate_combat(lua_State *L)
{
	...
	battle_context context(*resources::units, *att,
		*def, att_w, def_w, 0.0, NULL);
	...
	for (; !lua_isnoneornil(L, i); ++i)
	{
		if (gui2::tlistbox *l = dynamic_cast<gui2::tlistbox *>(w))
		{
			...
			if (v > n) {
				...
				gui2::string_map dummy;
				std::map<std::string, gui2::string_map> list_item_item;
				list_item_item.insert(std::make_pair("id", dummy));
				for (; n < v; ++n) {
					l->add_row(list_item_item);
				}
			}
			w = l->get_row_panel(v - 1);
		}
		else
		{
