...
void artifical::fallen(int a_side, unit* attacker)
{
	...
	for (std::vector<unit*>::iterator itor = cache_reside_troops.begin(); itor != cache_reside_troops.end() ; ++ itor) {
		...
		if (join_to_city) {
			if (current_troop->second().valid()) {
				...
				message = vgettext2("Find wise leader, $first would like to lead troop join in.", symbols);
			} else {
	...
	if (!attacker_team.is_human()) {
		...
		if (attacker_team.side() != team::empty_side) {
			game_events::show_hero_message(&heros_[hero::number_scout], NULL, vgettext2("$first occupy $second.", symbols), game_events::INCIDENT_FALLEN);
		} else {
			// in order to avoid overmany tip, close this.
			// game_events::show_hero_message(&heros_[hero::number_scout], NULL, vgettext2("$second is out of control, occupied by $first.", symbols), game_events::INCIDENT_FALLEN);
		}
	}
...
void show_mayor_message(hero_map& heros, hero& mayor, artifical& city, bool terminate)
{
	...
	if (terminate) {
		if (&mayor == rpg::h) {
			message = vgettext2("Your official of $city's mayor is terminated.", symbols);
		} else if (mayor.valid()) {
			symbols["hero"] = tintegrate::generate_format(mayor.name(), tintegrate::hero_color);
			message = vgettext2("$hero official of $city's mayor is terminated.", symbols);
		}
		incident = game_events::INCIDENT_INVALID;
	} else {
		if (&mayor == rpg::h) {
			message = vgettext2("Congratulate! You are appointed $city mayor.", symbols);
		} else if (mayor.valid()) {
			symbols["hero"] = tintegrate::generate_format(mayor.name(), tintegrate::hero_color);
			message = vgettext2("Congratulate! $hero is appointed $city mayor.", symbols);
		}
