...
void move_unit_spectator::add_seen_friend(const unit* u)
{
	seen_friends_.push_back(u);
}


void move_unit_spectator::add_seen_enemy(const unit* u)
{
	seen_enemies_.push_back(u);
}


const unit* move_unit_spectator::get_ambusher() const
{
	return ambusher_;
}


const unit* move_unit_spectator::get_failed_teleport() const
{
	return failed_teleport_;
}


const std::vector<const unit*>& move_unit_spectator::get_seen_enemies() const
{
	return seen_enemies_;
}


const std::vector<const unit*>& move_unit_spectator::get_seen_friends() const
{
...
const unit* move_unit_spectator::get_unit() const
{
	return unit_;
}
...
move_unit_spectator::move_unit_spectator(const unit_map& units)
	: ambusher_(NULL)
	, failed_teleport_(NULL)
	, seen_enemies_()
	, seen_friends_()
	, interrupted_waypoints_()
	, unit_(NULL)
{
}
...
void move_unit_spectator::set_ambusher(const unit* u)
{
	ambusher_ = u;
}


void move_unit_spectator::set_failed_teleport(const unit* u)
{
	failed_teleport_ = u;
}
...
void move_unit_spectator::set_unit(const unit* u)
{
	unit_ = u;
}
...
map_location best_attacker_location_from_range(const unit_map& units_, const unit& u, const std::string& range, const map_location& def)
{
	...
	for (int i = 0; i != adjacent_size; ++i) {
		unit* base = units_.find_unit(tiles[i], false);
		unit* overlay = units_.find_unit(tiles[i], true);
		if (!overlay && base) {
			if (base->wall()) {
				return tiles[i];
			}
	...
	effecting_tactic()
{
	...
	if (weapon) {
		...
		// conside wall
		if (!u.wall()) {
			const unit* u = units.find_unit(u_loc, false);
			if (u && u->wall()) {
				damage += damage / 10;
			}
		}
		if (!opp.wall()) {
			const unit* u = units.find_unit(opp_loc, false);
			if (u && u->wall()) {
				damage -= damage / 3;
			}
		}
...
bool attack::perform_hit(bool attacker_turn)
{
	...
	unit* find = units_.find_unit(attacker_stats->effecting_tactic, true);
...
bool attack::weapon_move(unit& u, const map_location& from, const map_location& to, std::set<map_location>& invalid_locs)
{
	...
	if (map.on_board(to) && !units_.valid2(to, true) && (u.movement_cost(map[to], &to) != unit_movement_type::UNREACHABLE) && !u_base_itor.valid()) {
...
std::pair<map_location, map_location> attack::perform()
{
	...
	if (duel_) {
		...
		if (point_in_rect_of_hexes(left_u.get_location().x, left_u.get_location().y, draw_area) || point_in_rect_of_hexes(right_u.get_location().x, right_u.get_location().y, draw_area)) {
			...
			std::string message;
			if (percentage > 50) {
				message = vgettext2("I take a duel with $second, and victory.", symbols);
			} else if (percentage < 50) {
				message = vgettext2("I take a duel with $second, but defeated.", symbols);
			} else {
				message = vgettext2("I take a duel with $second, and tied.", symbols);
			}
	...
	while (!anim_lock && duel_mode != NO_DUEL && !controller_.is_recovering(attacker.side()) && !resources::controller->has_network_player()) {
		...
		game_events::show_hero_message(attacker_is_human? &left: &right, NULL, vgettext2("$first is here, who dares to take a duel with me!", symbols), game_events::INCIDENT_DUEL);
	...
	for (size_t i = 0; i < attacker.adjacent_size_; i ++) {
		const unit* u2 = units_.find_unit(attacker.adjacent_[i], true);
		if (!u2 || attacker.side() != u2->side()) {
			continue;
		}
		...
		if (h2) {
			if (point_in_rect_of_hexes(attacker.get_location().x, attacker.get_location().y, draw_area) || point_in_rect_of_hexes(u2->get_location().x, u2->get_location().y, draw_area)) {
				unit_display::global_anim_2(anim2::REINFORCE, h1.image(true), h2->image(true), text);
			}
			a_.reinforced_ = true;
			break;
		}
	...
	if (!anim_lock && a_stats_->damage >= game_config::min_valid_damage) {
		...
		if (h2) {
			if (point_in_rect_of_hexes(attacker.get_location().x, attacker.get_location().y, draw_area) || point_in_rect_of_hexes(defender.get_location().x, defender.get_location().y, draw_area)) {
				unit_display::global_anim_2(anim2::INDIVIDUALITY, h2->image(true), "projectiles/strike-n-3.png", text);
			}
		...
		if (!((random + attacker.master().number_) % 2)) {
			...
			if (ret.first) {
				if (point_in_rect_of_hexes(attacker.get_location().x, attacker.get_location().y, draw_area) || point_in_rect_of_hexes(defender.get_location().x, defender.get_location().y, draw_area)) {
					unit_display::global_anim_2(anim2::REINFORCE, ret.second->image(true), ret.first->image(true), dgettext("wesnoth-hero", "hate"));
				}
				a_.hated_ = true;
			}
		}
	...
	if (h2) {
		...
		if (point_in_rect_of_hexes(attacker.get_location().x, attacker.get_location().y, draw_area) || point_in_rect_of_hexes(defender.get_location().x, defender.get_location().y, draw_area)) {
			if (h1 != h2) {
				unit_display::global_anim_2(anim2::REINFORCE, h1->image(true), h2->image(true), text, font::BAD_COLOR);
			} else {
				unit_display::global_anim_2(anim2::INDIVIDUALITY, h1->image(true), "projectiles/strike-n-3.png", text, font::BAD_COLOR);
			}
		}
		d_.reinforced_ = true;
	}
...
void carry_supplying(const team& current_team, unit_map& units, unit& provider)
{
	...
	for (int step = 0; step < 2; step ++) {
		...
		for (size_t adj = 0; adj < adjance_size; adj ++) {
			const map_location& loc = tiles[adj];
			unit* u2 = units.find_unit(loc, true);
			if (!u2) {
				continue;
			}
			unit& that = *u2;
	...
	for (std::map<map_location, std::vector<std::pair<artifical*, artifical*> > >::const_iterator it = road_locs.begin(); it != road_locs.end(); ++ it) {
		...
		for (std::vector<std::pair<artifical*, artifical*> >::const_iterator it2 = v.begin(); it2 != v.end(); ++ it2) {
			...
			if (a_side == b_side && a_side == side) {
				unit* u2 = units.find_unit(loc, true);
				if (u2 && current_team.is_enemy(u2->side())) {
					unit& u = *u2;
...
namespace {
			...
			std::set<map_location>* petrified_units = NULL)
	{
		...
		for(std::vector<map_location>::const_iterator it =
			...
			const unit* sighted = resources::units->find_unit(*it, true);
			if (sighted && (!sighted->invisible(*it) || !tm.is_enemy(sighted->side()))) {
		 ...
		 bool should_clear_shroud, bool is_replay)
{
	...
	unit* ui = units.find_unit(route.front(), true);
	unit& mover = *ui;
	...
	for (step = route.begin()+1; step != route.end(); ++step) {
		...
		if (expediting_city_cookie && units.find_unit(cur_loc, true) == expediting_city_cookie) {
		...
		int cost;
		unit* curr_node = units.find_unit(*step, false);
		if (curr_node && curr_node->wall2()) {
		...
		if (continue_move == false && preferences::interrupt_when_ally_sighted() == false) {
			...
			{
				const unit* u = units.find_unit(*it, true);
		...
		const unit* enemy_unit = units.find_unit(*step, true);
		if (enemy_unit) {
		...
		if (!enemy_unit) {
			const unit* base = units.find_unit(*step, false);
			if (!base || base->can_stand(*ui)) {
				...
				if (alerted_unit) {
					...
					if (move_spectator != NULL) {
						move_spectator->set_ambusher(units.find_unit(alerted_unit->get_location(), true));
					}
		...
		for (int s = 0; s < steps && !discovered_unit; s ++) {
			...
			for (size_t i = 0; i < adjacent_size; i ++) {
				...
				if (u && tm->is_enemy(u->side()) && u->invisible(u->get_location())) {
					...
					if (move_spectator != NULL) {
						move_spectator->set_ambusher(units.find_unit(adjacent_loc, true));
					}
					...
					if (!hides.cfgs.empty()) {
						...
						ambushed_string = _("Discover ambusher!");
					}
				}
			}
		}
	...
	while (steps.size() >= 2) {
		...
		if (!u->base()) {
			if (unit_is_city(u) && u->side() == ui->side() && (!expediting_city_cookie || units.find_unit(loc, true) != expediting_city_cookie)) {
	...
	for (int i = 0; i != 6; ++i) {
		const unit* it = units.find_unit(adjacent[i], true);
		if (it && tm->is_enemy(it->side()) && it->invisible(it->get_location())) {
	...
	ui = units.find_unit(steps.back(), true);
	ui->set_standing();
	...
	unit* maybe_ui = units.find_unit(steps.back(), true);
	...
	if (continue_move == false && seen_units.empty() == false) {
		...
		for(std::set<map_location>::const_iterator i = seen_units.begin(); i != seen_units.end(); ++i) {
			...
			const unit* u = units.find_unit(*i, true);
		...
		if(nfriends == 0 || nenemies == 0) {
			...
			message = vgettext2("Units sighted! ($friendphrase, $enemyphrase)", symbols);
...
/*
		if(steps.size() < route.size()) {
			...
			if(!hk.null()) {
				...
				message += "\n" + vgettext2("(press $hotkey to keep moving)", symbols);
			}
		}
*/
		...
		if (task == unit::TASK_TRADE) {
			...
			if (forward) {
				...
				if (move_spectator && !spectator_in_mover) {
					move_spectator->set_unit(units.find_unit(steps.front(), true));
				}
	...
	} else if (mover_is_transport && maybe_ui) {
		maybe_ui->set_block_turns(0);

		unit* it = units.find_unit(steps.back(), false);
		if (it && it->fort()) {
			do_supply(*tm, units, *maybe_ui, *unit_2_artifical(it));
			maybe_ui = units.find_unit(steps.back(), true);
			if (move_spectator && !spectator_in_mover) {
				move_spectator->set_unit(maybe_ui);
			}
			steps.clear();
	...
	if (!controller->is_recovering(team_num + 1) && alerted_unit) {
		// attack no defend!
		if (maybe_ui && !no_fightback_attack(*alerted_unit, *maybe_ui)) {
			move_spectator->set_unit(NULL);
		}
		event_mutated = true;

	} else if (!controller->is_recovering(team_num + 1) && !to_city && maybe_ui) {
		int random = rand();
...
std::pair<map_location, map_location> attack_enemy(unit* a_, unit* d_, int att_weapon, int def_weapon, bool move, bool constant_attacks)
{
	...
	unit* attacker = units_.find_unit(to_locs.first, true);
	if (attacker) {
		dialogs::advance_unit(*attacker, true);
	}

	unit* defender = units_.find_unit(to_locs.second, true);
	if (defender) {
...
bool unit_can_move(const unit &u)
{
	...
	for(int n = 0; n != 6; ++n) {
		if (resources::game_map->on_board(locs[n])) {
			const unit* i = resources::units->find_unit(locs[n], true);
			if (i) {
...
void get_random_card(team& t, game_display& disp, unit_map& units, hero_map& heros)
{
	...
	if (added && t.is_human()) {
		...
		game_events::show_hero_message(&heros[hero::number_scout], NULL, vgettext2("Get card: $first.", symbols), game_events::INCIDENT_CARD);
	}
}
...
void erase_random_card(team& t, game_display& disp, unit_map& units, hero_map& heros)
{
	...
	if (t.is_human()) {
		...
		game_events::show_hero_message(&heros[hero::number_scout], NULL, vgettext2("Lost card: $first.", symbols), game_events::INCIDENT_CARD);
	}
}

artifical* artifical_existed_ea(unit_map& units, const map_location& loc)
{
	unit* find = units.find_unit(loc, true);
	if (!find) {
		return NULL;
	}
...
void calculate_wall_tiles(unit_map& units, gamemap& map, artifical& owner, std::vector<map_location*>& keeps, std::vector<map_location*>& wall_vacants, std::vector<map_location*>& keep_vacants)
{
	...
	for (int i = 0; i != adjacent_size; ++ i) {
		// find count of keep
		// find vacant tile
		const unit* itor = units.find_unit(tiles[i], true);
		if (itor) {
		...
		itor = units.find_unit(tiles[i], false);
		if (itor) {
			if (itor->type() == keep_type) {
				keeps.push_back(&tiles[i]);
			}
...
void damage_range(unit_map& units, std::vector<team>& teams, unit& u, const std::string& type, const std::string& relative, int ratio)
{
	...
	for (int step = 0; step < 2; step ++) {
		...
		for (size_t adj = 0; adj != adjance_size; adj ++) {
			const map_location& loc = tiles[adj];
			unit* u_it = find_visible_unit(loc, u_team);
			if (!u_it) {
				continue;
			}
...
bool adjacent_has_enemy(const unit_map& units, const team& current_team, const unit& u)
{
	...
	for (int i = 0; i != adjacent_size; ++ i) {
		const unit* another = units.find_unit(tiles[i], true);
		if (another && current_team.is_enemy(another->side())) {
			return true;
		}
		another = units.find_unit(tiles[i], false);
		if (another && current_team.is_enemy(another->side())) {
			return true;
		}
	}
...
const ttechnology& browse_stratagem(display& disp, int stratagem_tag)
{
	...
	for (int n = stratagem_tag::min; n <= stratagem_tag::max; n ++) {
		...
		strstr.str("");
		strstr << s.name();
		items.push_back(gui2::tval_str(n, strstr.str()));
	}

	gui2::tcombo_box dlg(items, stratagem_tag);
	dlg.show(disp.video());

	return stratagem_tag::technology(dlg.selected_val());
}
...
std::string format_loc(const unit_map& units, const map_location& loc, int side)
{
	...
	if (to_city) {
		...
		const unit* base = units.find_unit(loc, false);
		if (base && base->fort()) {
...
void do_hate_relation(hero& h1, hero& h2, bool set)
{
	...
	if (set) {
		...
		message = vgettext2("Hate of national perdition! I will be mutually exclusive with $second.", symbols);
		sound::play_sound("spear.ogg");

	} else {
		incident = game_events::INCIDENT_ALLY;
		message = vgettext2("I'm happy! I got rid of hate with $second.", symbols);
	}
	if (!message.empty()) {
