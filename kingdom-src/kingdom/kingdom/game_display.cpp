...
void game_display::taccess_list::calculate_require_count(int side)
{
	...
	const gui2::tgrid::tchild* children = bar->child_begin();
	int childs = bar->childs();
	...
	for (int i = 0; i < childs; i ++) {
		...
		bar->set_child_visible(i, drawn_ticks != HIDDEN_TICKS);
	}
}
...
void game_display::taccess_list::reload(game_display& gui, int side)
{
	...
	gui2::tpoint unit_size = bar->get_unit_size();
	...
	if (type_ == TROOP) {
		for (unit_map::const_iterator it = units_.begin(); it != units_.end(); ++ it) {
			...
			widget = bar->create_child(null_str, null_str, u);
		...
		for (std::vector<artifical*>::const_iterator c_it = holden_cities.begin(); c_it != holden_cities.end(); ++ c_it) {
			...
			for (std::vector<hero*>::const_iterator it = fresh.begin(); it != fresh.end(); ++ it) {
				...
				widget = bar->create_child(null_str, null_str, h);
				bar->insert_child(*widget);
...
void game_display::taccess_list::insert(game_display& gui, int side, void* cookie)
{
	...
	gui2::tcontrol* widget = bar->create_child(null_str, null_str, cookie);
		...
		moving_dst_loc_()
{
	...
	if (controller) {
		...
		troop_list_.bar = dynamic_cast<gui2::treport*>(dlg.get_object("access-unit"));
		if (troop_list_.bar) {
			troop_list_.bar->tabbar_init(false, "surface", true);
			troop_list_.bar->tabbar_set_callback_show(boost::bind(&game_display::show_access_troop, this, _1, _2));
		}

		hero_list_.bar = dynamic_cast<gui2::treport*>(dlg.get_object("access-hero"));
		if (hero_list_.bar) {
			hero_list_.bar->tabbar_init(false, "surface", true);
			hero_list_.bar->parent()->set_visible(gui2::twidget::INVISIBLE);
		}
		...
		for (std::vector<map_location>::const_iterator it2 = it->second.begin(); it2 != it->second.end(); ++ it2) {
			...
			if (!city || city->get_location() != loc) {
				if (city && (city != a && city != b)) {
					...
					err << vgettext2("map error, road that from $from to $to exists other city: $other!", symbols);
...
void game_display::verify_access_troops() const
{
	...
	const gui2::tgrid::tchild* children = list->bar->child_begin();
	int childs = list->bar->childs();
...
void game_display::show_access_troop(gui2::treport* bar, gui2::tcontrol* widget)
{
	const gui2::tpoint& size = bar->get_unit_size();
...
{ 
	...
	if (list->bar) {
		list->bar->hide_children();
		list->bar->parent()->set_visible(gui2::twidget::INVISIBLE);
	}
	...
	if (list->bar) {
		list->bar->parent()->set_visible(gui2::twidget::VISIBLE);
		list->bar->replacement_children();
	}
}
...
{
	return in_game_? map_area(): anim2::rt.rect;
}
...
void game_display::post_change_resolution(const std::map<const std::string, bool>& actives)
{
	...
	if (current_list_type_ == taccess_list::TROOP) {
		require_invisible = hero_list_.bar;
	} else {
		require_invisible = troop_list_.bar;
	}
