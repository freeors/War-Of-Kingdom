	...
	connect_signal_mouse_left_click(
		*button
		, boost::bind(
			...
			, boost::ref(window)));
	strstr.str("");
	strstr << tintegrate::generate_format(_("Save Map"), "blue");
	...
	if (!param_) {
		...
		if (!local_only_ && associates.size() == 1) {
			...
			if (group.associates().empty()) {
				strstr << vgettext2("Require seting '$associate' in '$side'. Player that no agreement can be defender, ally agreement can be reinforce.", symbols);
			} else {
				strstr << vgettext2("No attackable player! Require seting '$associate' in '$side', and set at least one no agreement.", symbols);
			}
...
void tsiege_tent::save_map(twindow& window)
{
	...
	strstr.str("");
	strstr << vgettext2("Do you want to save $username|'s map to your city map?", symbols);
