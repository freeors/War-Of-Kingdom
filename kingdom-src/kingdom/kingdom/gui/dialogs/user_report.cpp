...
void tuser_report::pre_show(CVideo& video, twindow& window)
{
	...
	layers_.push_back(find_widget<tlistbox>(&window, "player_pass_list", false, true));
	layers_.push_back(find_widget<tlistbox>(&window, "rank_pass_list", false, true));
	layers_.push_back(find_widget<tlistbox>(&window, "employee_list", false, true));
	layers_.push_back(find_widget<tlistbox>(&window, "rank_score_list", false, true));
	layers_.push_back(find_widget<tlistbox>(&window, "title_list", false, true));
...
void tuser_report::fill_pass_table(tlistbox& list, twindow& window, const std::vector<http::pass_statistic>& passes, bool board)
{
	std::stringstream strstr;
	list.clear();
	BOOST_FOREACH (const http::pass_statistic& pass, passes) {
		...
		item["label"] = format_elapse_hms2(pass.duration);
...
{
	tlistbox& list = *layers_[EMPLOYEE_PAGE];
...
void tuser_report::detail_group(twindow& window, int page)
{
	tlistbox& list = *layers_[page];
	twidget* grid_ptr = list.get_row_panel(list.get_selected_row());
...
void tuser_report::fill_score_board(twindow& window)
{
	...
	tlistbox* list = layers_[RANK_SCORE_PAGE];
	list->clear();
...
void tuser_report::fill_title_board(twindow& window)
{
	...
	std::stringstream strstr;
	utils::string_map symbols;
	tlistbox* list = layers_[TITLE_BOARD_PAGE];
	list->clear();
