...
void modify_grid_with_data(twidget* grid, const std::map<std::string, string_map>& map)
{
...
void set_visible_if_exists(twidget* grid, const char* id, bool visible)
{
...
void tlobby_main::update_gamelist()
{
	...
	for (unsigned i = 0; i < lobby_info_.games().size(); ++i) {
		...
		twidget* grid = gamelistbox_->get_row_panel(gamelistbox_->get_item_count() - 1);
	...
	for (unsigned i = 0; i < lobby_info_.games().size(); ++i) {
		...
		if (game.display_status == game_info::NEW) {
			...
			if (game.display_status == game_info::UPDATED) {
				...
				twidget* grid = gamelistbox_->get_row_panel(list_i);
...
void tlobby_main::update_gamelist_header()
{
	...
	std::string games_string = vgettext2("Games: showing $num_shown out of $num_total", symbols);
...
void tlobby_main::adjust_game_row_contents(const game_info& game, int idx, twidget* grid)
{
...
void tlobby_main::fill_lobby(twindow& window)
{
	...
	gamelistbox_->set_callback_value_change(
			dialog_callback3<tlobby_main, tlistbox, &tlobby_main::gamelist_change_callback>);
...
void tlobby_main::gamelist_change_callback(gui2::twindow &/*window*/, tlistbox&, const int type)
{
	update_selected_game();
}
