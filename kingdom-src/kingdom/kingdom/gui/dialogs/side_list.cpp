...
void tside_list::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	lists_.push_back(find_widget<tlistbox>(&window, "military_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "diplomatism_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "gold_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "feature_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "technology_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "artifical_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "plan_list", false, true));

	for (std::vector<tlistbox*>::const_iterator it = lists_.begin(); it != lists_.end(); ++ it) {
		tlistbox& list = **it;
		list.set_callback_value_change(dialog_callback3<tside_list, tlistbox, &tside_list::hero_changed>);
	}
...
void tside_list::hero_changed(twindow& window, tlistbox& list, const int type)
{
	twidget* grid_ptr = list.get_row_panel(list.get_selected_row());
	unsigned int selected_side = dynamic_cast<ttoggle_panel*>(grid_ptr)->get_data();
...
void tside_list::fill_table(tlistbox& list, int catalog)
{
