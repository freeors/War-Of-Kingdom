...
void ttroop_list::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	for (std::vector<tlistbox*>::const_iterator it = lists_.begin(); it != lists_.end(); ++ it) {
		tlistbox& list = **it;
		list.set_callback_value_change(dialog_callback3<ttroop_list, tlistbox, &ttroop_list::city_changed>);
	}
...
void ttroop_list::city_changed(twindow& window, tlistbox& list, const int type)
{
	if (candidate_troops_.empty()) {
		return;
	}
...
void ttroop_list::fill_table(tlistbox& list, int catalog)
{
...
bool ttroop_list::compare_row(twidget& row1, twidget& row2)
{
...
void ttroop_list::sort_column(tbutton& widget, int catalog)
{
