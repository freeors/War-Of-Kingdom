...
void trecruit::type_selected2(twindow& window, tlistbox& list, const int type)
{
	twindow::tinvalidate_layout_blocker invalidate_layout_blocker(window);
	type_selected(window, list);
}

void trecruit::type_selected(twindow& window, tlistbox& list, const int type)
{
...
void trecruit::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	list->set_callback_value_change(dialog_callback3<trecruit, tlistbox, &trecruit::type_selected2>);
...
void trecruit::catalog_page(twindow& window, int page, bool swap)
{
	if (!page_panel_) {
		return;
	}
...
void trecruit::switch_type_internal(twindow& window)
{
	...
	for (std::vector<const unit_type*>::const_iterator it = recruits.begin(); it != recruits.end(); ++it) {
		...
		twidget* grid_ptr = list->get_row_panel(unit_types_.size());
