...
const tgroup& tmp_side_creator::saved_allow_username(const std::string& username)
{
	...
	if (choice == runtime_groups::gs.end()) {
		...
		std::string message = vgettext2("The save game reserved for $allow, don't support $exclude.", i18n_symbols);
...
*/
	...
	if (!era_cfg) {
		if (!params_.saved_game) {
			...
			throw config::error(vgettext2("Cannot find era $era", i18n_symbols));
		}
	...
	if(level_["objectives"].empty()) {
		level_["objectives"] = std::string("*") + _("Victory:") + "\n. " + _("Defeat enemy leader(s)");
	}
}
	...
	, changed_(false)
{
	...
	twidget* grid_ptr = sides_table->get_row_panel(index_);
