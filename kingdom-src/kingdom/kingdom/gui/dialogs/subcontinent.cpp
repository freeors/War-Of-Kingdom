...
void tsubcontinent::subcontinent_selected(twindow& window, tlistbox& list, const int type)
{
	int selected_row = list.get_selected_row();
	switch_to_subcontinent(window, params_[selected_row]);
}

void tsubcontinent::city_selected(twindow& window, tlistbox& list, const int type)
{
	int selected_row = list.get_selected_row();
	twidget* grid_ptr = list.get_row_panel(selected_row);
	unsigned int cookie = dynamic_cast<ttoggle_panel*>(grid_ptr)->get_data();
...
void tsubcontinent::refresh_title_flag(twindow& window) const
{
	...
	strstr.str("");
	strstr << _("Crusage for subcontinent");
...
void tsubcontinent::pre_show(CVideo& video, twindow& window)
{
	...
	list->set_callback_value_change(dialog_callback3<tsubcontinent, tlistbox, &tsubcontinent::subcontinent_selected>);
...
void tsubcontinent::switch_to_subcontinent(twindow& window, tparam& param)
{
	...
	tlistbox& list = find_widget<tlistbox>(&window, "city_table", false);
	list.set_callback_value_change(dialog_callback3<tsubcontinent, tlistbox, &tsubcontinent::city_selected>);
}
...
void tsubcontinent::repair(twindow& window)
{
	...
	symbols["do"] = tintegrate::generate_format(_("Repair"), "yellow");
	std::string message = vgettext2("Spend $score score may increase 10 endurance.\nDo you want to spend $score score to $do?", symbols);
...
void tsubcontinent::discard(twindow& window)
{
	...
	std::string message = vgettext2("Do you want to discard $city?", symbols);
	int res = gui2::show_message(disp_.video(), "", message, gui2::tmessage::yes_no_buttons);
	if (res == gui2::twindow::CANCEL) {
		return;
	}
