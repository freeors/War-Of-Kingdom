...
tgroup2::tgroup2(display& disp, hero_map& heros, const config& game_config, tgroup& group, bool browse)
	...
	, employees_()
	, fix_filled_(false)
	, employee_filled_(false)
	, associate_filled_(false)
{
}
...
void tgroup2::update_map(twindow& window, tlistbox& list, const int type)
{
...
void tgroup2::type_selected(twindow& window, tlistbox& list, const int type)
{
	twidget* grid_ptr = list.get_row_panel(list.get_selected_row());
	selected_number_ = dynamic_cast<ttoggle_panel*>(grid_ptr)->get_data();

	gui2::tbutton* button = find_widget<tbutton>(&window, "fix_upgrade", false, true);
...
void tgroup2::refresh_title_flag(twindow& window) const
{
	...
	strstr << vgettext2("$leader's side", symbols);
...
void tgroup2::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	lists_.push_back(find_widget<tlistbox>(&window, "fix_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "employee_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "associate_list", false, true));
...
bool tgroup2::text_box_int(twindow& window, const std::string& id, const std::string& name, int& value, int min, int max)
{
	...
	if (value < min || value > max) {
		...
		err.str("");
		err << vgettext2("Range is [$min, $max], invalid '$key' value:", symbols) << " " << str;
...
void tgroup2::exchange(twindow& window, bool score_2_coin)
{
	...
	std::string message = vgettext2("Do you want to exchange $from to $to?", symbols);
	...
	message = vgettext2("$do successfully!", symbols);
...
void tgroup2::fill_2_least(twindow& window)
{
	...
	if (coin_income || score_income) {
		...
		message = vgettext2("May get $coin coin and $score score, do you want to redraw $count hero?", symbols);
		int res = gui2::show_message(disp_.video(), "", message, gui2::tmessage::yes_no_buttons);
		if (res == gui2::twindow::CANCEL) {
			return;
		}
...
void tgroup2::draw(twindow& window)
{
	...
	if (sum_score(group.coin(), group.score()) < game_config::score_used_draw) {
		...
		symbols["do"] = tintegrate::generate_format(_("group^Draw"), "yellow");
...
void tgroup2::employ_employee(twindow& window)
{
	...
	if (ret) {
		...
		tlistbox& list = *lists_[EMPLOYEE_LIST];
...
void tgroup2::upgrade(twindow& window)
{
	...
	if (current_page_ == BASE_PAGE) {
		...
		upgrade_description << vgettext2("Upgrade to $to", symbols);
...
void tgroup2::discard_member(twindow& window)
{
	...
	symbols["coin"] = tintegrate::generate_format(coin_income, "green");
	symbols["score"] = tintegrate::generate_format(score_income, "green");
	strstr << vgettext2("May get $coin coin and $score score, do you want to discard $name?", symbols) << "\n\n";
	symbols["location"] = tintegrate::generate_format(_("Exile"), "yellow");
	if ((int)group_.exiles().size() < game_config::max_exile) {
		strstr << vgettext2("$name will put into $location.", symbols);
	} else {
		strstr << vgettext2("$location is full, $name will lose.", symbols);
	}
...
void tgroup2::fill_member_table(twindow& window, const std::vector<tgroup::tmember*>& members, int cursel)
{
	...
	list->set_callback_value_change(dialog_callback3<tgroup2, tlistbox, &tgroup2::type_selected>);
	...
	button = find_widget<tbutton>(&window, "fix_upgrade", false, true);
	...
	button = find_widget<tbutton>(&window, "fix_detail", false, true);
...
void tgroup2::refresh_employee_other(twindow& window)
{
	...
	button = find_widget<tbutton>(&window, "employee_upgrade", false, true);
...
void tgroup2::employee_selected(twindow& window, tlistbox& list, const int type)
{
	twidget* grid_ptr = list.get_row_panel(list.get_selected_row());
	selected_number_ = dynamic_cast<ttoggle_panel*>(grid_ptr)->get_data();
...
void tgroup2::fill_employee_table(twindow& window, int cursel)
{
	...
	list->set_callback_value_change(dialog_callback3<tgroup2, tlistbox, &tgroup2::employee_selected>);
	...
	tbutton* button = find_widget<tbutton>(&window, "employee_detail", false, true);
	button->set_active(list->get_item_count()? true: false);
...
void tgroup2::fill_fix_other(twindow& window)
{
	std::stringstream strstr;
...
void tgroup2::refresh_leader_noble(twindow& window)
{
	...
	tbutton* button = find_widget<tbutton>(&window, "base_upgrade", false, true);
	button->set_active(noble < unit_types.max_noble_level());
}

void tgroup2::associate_selected(twindow& window, tlistbox& list, const int type)
{
...
void tgroup2::insert_associate(twindow& window)
{
	...
	strstr.str("");
	if (username.empty()) {
		strstr << _("Username doesn't allow empty!");
	} else if (username == group_.leader().name()) {
		strstr << _("Username cannot be myself!");
	} else {
		...
		for (size_t n = 0; n < associates.size(); n ++) {
			...
			if (a.username == username) {
				strstr << _("Username has existd in associate list, cannot append!");
				break;
			}
		}
	}
	...
	if (m.uid < 0) {
		...
		strstr << vgettext2("$username doesn't exist!", symbols);
		gui2::show_message(disp_.video(), "", strstr.str());
		return;
	}
...
void tgroup2::fill_associate_table(twindow& window, int cursel)
{
	std::stringstream strstr;
	utils::string_map symbols;
	tlistbox* list = lists_[ASSOCIATE_LIST];
	list->clear();
	...
	for (std::vector<tgroup::tassociate>::iterator it = group_.associates().begin(); it != group_.associates().end(); ++ it) {
		...
		if (a.t) {
			...
			if (a.agreement == tgroup::tassociate::requestally) {
				...
				strstr << vgettext2("Initiation time: $time", symbols);
			} else if (a.agreement == tgroup::tassociate::requestterminate) {
				int terminate_threshold = 2 * 24 * 3600;
				time_t now = time(NULL);
				symbols["space"] = tintegrate::generate_format(format_elapse_hms2(a.t + terminate_threshold - now), "yellow");
				strstr << vgettext2("Termiante automatically after $space", symbols);
			} else {
	...
	list->set_callback_value_change(dialog_callback3<tgroup2, tlistbox, &tgroup2::associate_selected>);
	...
	button = find_widget<tbutton>(&window, "associate_detail", false, true);
	button->set_active(list->get_item_count()? true: false);
...
void tgroup2::fill_city(twindow& window)
{
	...
	ttext_box* box = find_widget<ttext_box>(&window, "text_box_city", false, true);
	box->set_value(group_.city().name());
	...
	strstr << vgettext2("1: Select the map want to set.\n2: Play $layout, execute $save.", symbols);
	label->set_label(strstr.str());	

	tlistbox& list = find_widget<tlistbox>(&window, "map_list", false);
	list.set_callback_value_change(dialog_callback3<tgroup2, tlistbox, &tgroup2::update_map>);
	...
	string_map list_item;
	std::map<std::string, string_map> list_item_item;
	list_item.insert(std::make_pair("label", tintegrate::generate_format(_("Using map"), "yellow")));
	...
	BOOST_FOREACH (const config &map, game_config_.child_range("multiplayer")) {
		...
		list_item.insert(std::make_pair("label", map["name"].str()));
		list_item_item.insert(std::make_pair("id", list_item));
		list.add_row(list_item_item);

		grid_ptr = list.get_row_panel(list.get_item_count() - 1);
		toggle = dynamic_cast<ttoggle_panel*>(grid_ptr);
		toggle->set_data(index);
	}
	update_map(window, list);
}
...
void tgroup2::fill_employee(twindow& window)
{
	...
	if (!employee_filled_) {
		fill_employee_other(window);
		employee_filled_ = true;
	}
}

void tgroup2::fill_employee_other(twindow& window)
{
	std::stringstream strstr;
	tlabel* label = find_widget<tlabel>(&window, "remark", false, true);
	strstr << tintegrate::generate_format(_("PS"), "red") << " ";
	strstr << tintegrate::generate_format(_("If defend city fail, you will lose a employee."), "yellow");
	label->set_label(strstr.str());
	...
	button = find_widget<tbutton>(&window, "employee_detail", false, true);
	connect_signal_mouse_left_click(
		*button
		, boost::bind(
		&tgroup2::detail
...
void tgroup2::fill_associate(twindow& window)
{
	...
	if (!associate_filled_) {
		fill_associate_other(window);
		associate_filled_ = true;
	}
}

void tgroup2::fill_associate_other(twindow& window)
{
	std::stringstream strstr;
