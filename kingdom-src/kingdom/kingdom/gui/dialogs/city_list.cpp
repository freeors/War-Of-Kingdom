...
void tcity_list::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	lists_.push_back(find_widget<tlistbox>(&window, "units_list", false, true));
	lists_.push_back(find_widget<tlistbox>(&window, "interior_list", false, true));

	for (std::vector<tlistbox*>::const_iterator it = lists_.begin(); it != lists_.end(); ++ it) {
		tlistbox& list = **it;
		list.set_callback_value_change(dialog_callback3<tcity_list, tlistbox, &tcity_list::city_changed>);
	}
...
void tcity_list::city_changed(twindow& window, tlistbox& list, const int type)
{
...
void tcity_list::fill_table(tlistbox& list, int catalog)
{
...
bool tcity_list::compare_row(twidget& row1, twidget& row2)
{
...
void tcity_list::sort_column(tbutton& widget, int catalog)
{
