...
private:
	...
	void update_map(twindow& window, tlistbox& list, const int type = twidget::drag_none);
	...
	enum {BASE_PAGE, CITY_PAGE, FIX_PAGE, ROAM_PAGE, ASSOCIATE_PAGE, NONE_PAGE};
	enum {FIX_LIST, EMPLOYEE_LIST, ASSOCIATE_LIST};
	...
	void employee_selected(twindow& window, tlistbox& list, const int type = twidget::drag_none);
	void fill_employee_table(twindow& window, int cursel);
	
	void associate_selected(twindow& window, tlistbox& list, const int type = twidget::drag_none);
	...
	void fill_fix_other(twindow& window);
	void fill_employee_other(twindow& window);
	void fill_associate_other(twindow& window);
...
private:
	...
	bool fix_filled_;
	bool employee_filled_;
	bool associate_filled_;
};
