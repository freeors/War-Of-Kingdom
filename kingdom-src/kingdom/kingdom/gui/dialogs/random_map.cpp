...
void trandom_map::pre_show(twindow& window)
{
	...
	list.set_callback_value_change(dialog_callback3<trandom_map, tlistbox, &trandom_map::update_map>);

	// Load option (might turn it into a button later).
	string_map list_item;
	std::map<std::string, string_map> list_item_item;
	if (mode_ != mode_tag::TOWER) {
		list_item.insert(std::make_pair("label", tintegrate::generate_format(_("Load Game"), "yellow")));
		list_item.insert(std::make_pair("tooltip", _("Load Game...")));
		list_item_item.insert(std::make_pair("id", list_item));
		list.add_row(list_item_item);

		twidget* toggle = list.get_row_panel(list.get_item_count() - 1);
		toggle->set_cookie(reinterpret_cast<void*>(LOAD_GAME));

		if (!local_only_) {
			list_item.clear();
			list_item_item.clear();
			list_item.insert(std::make_pair("label", tintegrate::generate_format(dsgettext("wesnoth-multiplayer", "2p_siege_map"), "green")));
			list_item.insert(std::make_pair("tooltip", _("Siege map")));
			list_item_item.insert(std::make_pair("id", list_item));
			list.add_row(list_item_item);

			toggle = list.get_row_panel(list.get_item_count() - 1);
			toggle->set_cookie(reinterpret_cast<void*>(SIEGE_MODE));
		}
	}
	...
	BOOST_FOREACH (const config &map, cfg_.child_range("multiplayer"))
	{
		...
		if (map["allow_new_game"].to_bool(true)) {
			...
			list_item.insert(std::make_pair("tooltip", map["name"].str()));
			list_item_item.insert(std::make_pair("id", list_item));
			list.add_row(list_item_item);

			twidget* toggle = list.get_row_panel(list.get_item_count() - 1);
			toggle->set_cookie(reinterpret_cast<void*>(BASE + index - 1));
		}
	}
...
void trandom_map::update_map(twindow& window, tlistbox& list, const int type)
{
	twidget* grid_ptr = list.get_row_panel(list.get_selected_row());
	int select = (int)reinterpret_cast<long>(grid_ptr->cookie());
