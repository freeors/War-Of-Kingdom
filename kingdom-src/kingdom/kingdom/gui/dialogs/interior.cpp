...
void tinterior::type_selected(twindow& window, tlistbox& list, const int type)
{
...
void tinterior::refresh_tooltip(twindow& window)
{
	...
	for (std::vector<department>::const_iterator d_it = departments_.begin(); d_it != departments_.end(); ++ d_it) {
		twidget* grid_ptr = list->get_row_panel(d_it->type_);
...
void tinterior::appoint(twindow& window)
{
	...
	tlistbox* hero_table = lists_[current_page_];
	tcontrol* control;
	for (int index = 0; index < (int)fresh_heros_.size(); index ++) {
		twidget* grid_ptr = hero_table->get_row_panel(index);
...
void tinterior::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	list->set_callback_value_change(dialog_callback3<tinterior, tlistbox, &tinterior::type_selected>);
...
void tinterior::catalog_page(twindow& window, int page, bool swap)
{
	if (!page_panel_) {
		return;
	}
