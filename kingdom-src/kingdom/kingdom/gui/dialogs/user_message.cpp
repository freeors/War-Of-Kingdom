...
{
	const std::string unknown_standard_message = _("Not conform to the format message");
	...
	if (msgid == msgid_signin) {
		...
		if (subid == 0) {
			...
			parsed = vgettext2("You continue to sign in $continue days, reward $coin coin and $score score.", symbols);
		} else if (subid == 1) {
			if (vstr.size() != 4) {
				return unknown_standard_message;
			}
			...
			parsed = vgettext2("You have broken $break days, exceeded $max_break, and result to clear Sign in data.", symbols);
		} else {
			return unknown_standard_message;
		}
		...
		if (subid == 0) {
			...
			if (title == title_score) {
				...
				parsed = vgettext2("You hold $coin2 coin and $score2 score, most score, so win $title as an honour. Reward $coin coin and $score score.", symbols);

			} else if (title == title_attacker) {
				symbols["count"] = vstr[5];
				parsed = vgettext2("In the last $days days, you attack other city to $count times, most times, so win $title as an honour. Reward $coin coin and $score score.", symbols);

			} else if (title == title_defender) {
				symbols["count"] = vstr[5];
				parsed = vgettext2("In the last $days days, your city was attacked to $count times, most times, so win $title as an honour. Reward $coin coin and $score score.", symbols);

			} else if (title == title_rpg) {
				symbols["count"] = vstr[5];
				parsed = vgettext2("In the last $days days, you pass $count times RPG scenario, most times, so win $title as an honour. Reward $coin coin and $score score.", symbols);

			} else if (title == title_tower) {
				symbols["count"] = vstr[5];
				parsed = vgettext2("In the last $days days, you pass $count times Tower scenario, most times, so win $title as an honour. Reward $coin coin and $score score.", symbols);
		...
		if (subid == 0) {
			...
			parsed = vgettext2("Inapp-purchase successfully! Increase $day days VIP membership.", symbols);
			...
			parsed = vgettext2("Inapp-purchase successfully! Increase $coin coin and $score score.", symbols);
			...
			parsed = vgettext2("System modified your data: $coin coin, $score score. If you have any questions, please contact $contact.", symbols);
		} else {
			return unknown_standard_message;
		}
...
{
	// std::string initial_str = _("Announcement content");
	...
	if (m.uid >= 0) {
		gui2::show_message(disp_.video(), "", _("Send successfully!"));
...
void tuser_message::message_selected(twindow& window, tlistbox& list, const int type)
{
...
void tuser_message::fill_message_table(twindow& window, const std::vector<http::tmessage_record>& messages)
{
	...
	list->set_callback_value_change(dialog_callback3<tuser_message, tlistbox, &tuser_message::message_selected>);
	if (!messages_.empty()) {
...
void tuser_message::detail_employee(twindow& window)
{
	...
	int number = dynamic_cast<ttoggle_panel*>(grid_ptr)->get_data();
...
void tuser_message::detail_group(twindow& window)
{
	...
	twidget* grid_ptr = list.get_row_panel(list.get_selected_row());
	int uid = dynamic_cast<ttoggle_panel*>(grid_ptr)->get_data();
