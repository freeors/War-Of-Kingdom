...
void tbuild_ea::tactic_selected(twindow& window, tlistbox& list, const int type)
{
...
void tbuild_ea::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	for (std::vector<const unit_type*>::const_iterator it = can_build_ea_.begin(); it != can_build_ea_.end(); ++ it) {
		...
		if (tent::mode != mode_tag::LAYOUT) {
			...
			strstr << vgettext2("Increase $item every day.", symbols);
		}
		list_item["label"] = strstr.str();
	...
	list.set_callback_value_change(dialog_callback3<tbuild_ea, tlistbox, &tbuild_ea::tactic_selected>);

	tactic_selected(window, list);
}
