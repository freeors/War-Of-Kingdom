...
void trpg_detail::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	list->set_callback_value_change(dialog_callback3<trpg_detail, tlistbox, &trpg_detail::type_selected>);
...
void trpg_detail::human_troops(twindow& window)
{
	...
	gui2::ttroop_detail dlg(gui_, teams_, units_, heros_, partial_troops, _("RPG Troop"));
	try {
		dlg.show(gui_.video());
