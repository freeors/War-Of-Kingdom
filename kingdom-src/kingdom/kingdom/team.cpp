...
void show_noble_message(unit_map& units, hero_map& heros, hero& h, const tnoble& noble, bool terminate)
{
	...
	if (terminate) {
		if (&h == rpg::h) {
			message = vgettext2("Your noble of $noble is terminated.", symbols);
		} else {
			symbols["hero"] = tintegrate::generate_format(h.name(), tintegrate::hero_color);
			message = vgettext2("$hero noble of $noble is terminated.", symbols);
		}
		incident = game_events::INCIDENT_INVALID;
	} else {
		if (&h == rpg::h) {
			message = vgettext2("Congratulate! You are appointed $noble.", symbols);
		} else {
			symbols["hero"] = tintegrate::generate_format(h.name(), tintegrate::hero_color);
			message = vgettext2("Congratulate! $hero is appointed $noble.", symbols);
		}
	...
	if (find != half_technologies_.end()) {
		...
		if (xp >= max_experience) {
			...
			if (ing_technology_) {
				...
				message = vgettext2("$side finished researching $end, will begin to research $begin!", symbols);
			} else {
				message = vgettext2("$side finished researching $end, and have gotten all technology!", symbols);
			}
...
void team::set_ally(int n, bool alignment, bool dialog, bool adjust)
{
	...
	if (dialog && is_human()) {
		...
		if (alignment) {
			message = vgettext2("$first and $second concluded treaty of alliance.", symbols);
		} else {
			message = vgettext2("$first and $second terminate treaty of alliance.", symbols);
		}
			...
			for (std::set<int>::const_iterator it_ally = it->allies_.begin(); it_ally != it->allies_.end(); ++ it_ally) {
				...
				if (dialog) {
					...
					if (it->type_ == strategy::AGGRESS) {
						message = vgettext2("End aggressing upon $city, $first and $second terminate treaty of alliance.", symbols);
					} else if (it->type_ == strategy::DEFEND) {
						message = vgettext2("End defending $city, $first and $second terminate treaty of alliance.", symbols);
					} else {
...
std::string team::form_results_of_battle_tip(const std::string& prefix) const
{
	...
	if (!prefix.empty()) {
		strstr << prefix;
		strstr << "\n\n";
		strstr << tintegrate::generate_format(_("Results of battle"), "", 0, true, true);
		strstr << tintegrate::generate_img("misc/tintegrate-split-line.png") << "\n";
	}
	strstr << tintegrate::generate_format(_("Cause damage"), "green") << ": ";
	strstr << cause_damage_ << "\n";

	strstr << tintegrate::generate_format(_("Been damage"), "green") << ": ";
	strstr << been_damage_ << "\n";

	strstr << tintegrate::generate_format(_("Defeat units"), "green") << ": ";
	strstr << defeat_units_ << "\n";

	strstr << tintegrate::generate_format(_("scenario^Perfect turns"), "green") << ": ";
...
void team::find_treasure(hero_map& heros, play_controller& controller, int pos)
{
	...
	game_events::show_hero_message(&heros[hero::number_scout], NULL, vgettext2("Find treasure: $first($second)!", symbols), game_events::INCIDENT_CARD);
}
