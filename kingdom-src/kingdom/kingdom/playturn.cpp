/* $Id: playturn.cpp 47751 2010-11-29 12:23:14Z shadowmaster $ */
/*
		...
		network::connection from, std::deque<config>& backlog, bool skip_replay)
{
	...
	if(cfg["side_drop"] != "") {
		...
		// We want to give host chance to decide what to do for side
		{
			...
			{
				t_vars["player"] = ob;
				options.push_back(vgettext2("Replace with $player", t_vars));
			...
			const std::string msg =  vgettext2("$player has left the game. What do you want to do?", t_vars);
	...
	if (cfg.child("notify_next_scenario")) {
		resources::screen->set_theme_object_active("endturn", true);
