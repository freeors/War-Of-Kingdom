...
void mouse_handler::left_mouse_up(int x, int y, bool motions, const bool browse)
{
	if (motions) {
		return;
	}
	...
	if (!browse && !commands_disabled && attack_from.valid()) {
		...
		if (!path.destinations.contains(hex)) {
			...
			if (clicked_selfcity) {
				...
				message = vgettext2("Do you really command $troop to enter $city?", symbols);
			} else if(map_.is_village(hex)) {
				message = vgettext2("Do you really command $troop to move to village?", symbols);
			} else {
				std::stringstream str;
				str << "(" << hex.x + 1 << ", " << hex.y + 1 << ")";
				symbols["loc"] = str.str();
				message = vgettext2("Do you really command $troop to move to $loc?", symbols);
			}
...
map_location mouse_handler::move_unit_along_current_route(const std::vector<map_location>& caller_steps, bool check_shroud, bool attackmove)
{
	...
	SDL_WarpMouseInWindow(SDL_GetMouseFocus(), gui_->get_location_x(dst) + display::initial_zoom / 2, gui_->get_location_y(dst) + display::initial_zoom / 2);
#endif
