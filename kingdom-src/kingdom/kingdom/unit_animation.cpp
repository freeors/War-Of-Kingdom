		...
		, int value2)
{
	...
	tmp.cycles = cycles;
	tmp.anim = animated_unit->choose_animation(*disp, src, event, dst, value, hit_type, attack, second_attack, value2);
	if (!tmp.anim) {
		return;
	}
		...
		, int value2)
{
	...
	unit_animation* anim = dynamic_cast<unit_animation*>(animated_unit->get_animation());
	if (anim && !anim->animation_finished_potential() && anim->matches(*disp,src,dst,animated_unit,event,value,hit_type,attack,second_weapon,value2) >unit_animation::MATCH_FAIL) {
		anim_elem tmp;
		tmp.my_unit = animated_unit;
		tmp.text = text;
		tmp.text_color = text_color;
