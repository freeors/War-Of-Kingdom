...
bool commerce_protection(display& disp, const std::string& function)
{
	...
	strstr << vgettext2("Because of product commercialization, $function does not open source code.", symbols);
	strstr << "\n\n";
	const std::string postscript = _("Please do not use self-compile project to commerce, for example upload data to Data Server.");
...
bool http_agent::do_prepare(bool check_network, bool quiet)
{
	if (!game_config::server_matched) {
		std::string errmsg = _("Invalid access! You modified resources files, or check if your version is the newest.");
	...
	bool ret = network_connect_dialog(disp_, "", game_config::bbs_server.host, game_config::bbs_server.port, quiet);
...
bool http_agent::access_denied(const config& cfg, const char* buf, int len) const
{
	...
	if (cfg["__status"].to_int() != 200) {
		...
		i18n_symbols["account"] = tintegrate::generate_format(_("Register, config account"), "yellow");
		errmsg = vgettext2("Invalid access! Please check your $server login information. To set up: Press $account in \"Player profile\" Setting.", i18n_symbols);

	} else if (len >= (int)version_absent.size() && !memcmp(buf, version_absent.c_str(), version_absent.size())) {
		i18n_symbols["server"] = tintegrate::generate_format(game_config::bbs_server.name, "green");
		errmsg = vgettext2("Invalid access! pre_kingdom_version table on $server isn't ready.", i18n_symbols);

	} else if (len >= (int)error_checksum.size() && !memcmp(buf, error_checksum.c_str(), error_checksum.size())) {
		game_config::server_matched = false;
		i18n_symbols["server"] = tintegrate::generate_format(game_config::bbs_server.name, "green");
		errmsg = vgettext2("Invalid access! You modified resources files, or check if your version is the newest.", i18n_symbols);

	} else if (len >= (int)error_security.size() && !memcmp(buf, error_security.c_str(), error_security.size())) {
		i18n_symbols["server"] = tintegrate::generate_format(game_config::bbs_server.name, "green");
		errmsg = vgettext2("Invalid access! Security error, someone maybe attack cipher.", i18n_symbols);

	} else if (len >= (int)error_secretkey.size() && !memcmp(buf, error_secretkey.c_str(), error_secretkey.size())) {
		i18n_symbols["server"] = tintegrate::generate_format(game_config::bbs_server.name, "green");
		errmsg = vgettext2("Invalid access! Secret key error, Secret key on $server isn't ready.", i18n_symbols);

	} else if (len >= (int)error_timestamp.size() && !memcmp(buf, error_timestamp.c_str(), error_timestamp.size())) {
		i18n_symbols["server"] = tintegrate::generate_format(game_config::bbs_server.name, "green");
		errmsg = vgettext2("Invalid access! Timestamp error. Check whether login a username on more device at same time, or restart game.", i18n_symbols);

	} else if (len > (int)html_prefix.size() * 2 && !memcmp(buf, html_prefix.c_str(), html_prefix.size())) {
		errmsg = vgettext2("Send a command that server doesn't support!\n\nPlease check if your version is the newest.", i18n_symbols);

	} else if (len < (int)magic_.size() || memcmp(buf, magic_.c_str(), magic_.size())) {
		errmsg = vgettext2("Receive unkonwn data!\n\nPlease check if your version is the newest.", i18n_symbols);
...
bool http_agent::do_register(bool check_exist)
{
	...
	trequest request(*this, "register", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);

	std::vector<char> buf;
	network_receive_dialog(disp_, "", buf, sock_);

	ret = true;
	config data;
	int content_start = tlobby::thttp_sock::http_2_cfg(buf, data);
...
membership http_agent::do_session(bool quiet)
{
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		if (member.uid < 0 && !quiet) {
			gui2::show_message(disp_.video(), "", _("Receive unkonwn data!\n\nPlease check if your version is the newest."));
		}
	}
	return member;
}
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		if (content_length <= 8) {
			...
			if (!quiet) {
				gui2::show_message(disp_.video(), "", 
					vgettext2("No avater on $server. You can upload $middle and $small to custom visualization.", i18n_symbols));
			}
			return false;
		}
		...
		if (middle_size) {
			...
			if (surf->w == 180 && surf->h == 256) {
				...
				i18n_symbols["server"] = tintegrate::generate_format(game_config::bbs_server.name, "green");
				gui2::show_message(disp_.video(), "", vgettext2("Download $name fail! $name format (size: $size, type: png), please check uploaded file on $server.", i18n_symbols));
			}
		}
		strstr.str("");
...
bool http_agent::do_uploadsave(const std::string& name)
{
	...
	network_send_dialog(disp_, "", request.buf,  request.size, sock_, 0);
	free(file_data);
...
std::string http_agent::do_downloadsave(int sid)
{
	...
	std::vector<char> buf;
	network_receive_dialog(disp_, "", buf, sock_, 0);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		if (content_length == 8 + name_size) {
			...
			err << vgettext2("$file maybe deleted by server. Download fail!", symbols);
...
membership http_agent::do_uploadpass(const pass_statistic& stat)
{
	...
	content << stat.create_time << "&" << stat.duration << "&" << game_config::wesnoth_version.transfer_format().first << "&";
	content << stat.coin << "&" << stat.score << "&" << stat.type << "&" << stat.hash << "&";
	content << stat.subcontinent.first << "&";
	content << stat.subcontinent.second;
	
	trequest request(*this, "uploadpass", content.str());
	network_send_dialog(disp_, "", request.buf,  request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		if (str == "score_error") {
			symbols["score"] = tintegrate::generate_format(stat.score, "red");
			err << vgettext2("This save's score($score) is invalid, cannot upload!", symbols);
			
		} else if (str == duplicate) {
			symbols["create"] = tintegrate::generate_format(format_time_date(stat.create_time), "red");
			err << vgettext2("Had uploaded pass that create at $create, cannot upload again!", symbols);

		} else if (str == "error_param") {
			err << vgettext2("param error. cannot upload!", symbols);
		}
		if (!err.str().empty()) {
			gui2::show_message(disp_.video(), "", err.str());
			return m;
...
membership http_agent::do_uploadsiege(const tsiege_record& rec)
{
	...
	std::stringstream content;
	content << rec.defender << "&" << rec.atk_reinforce << "&" << rec.def_reinforce << "&";
	content << rec.create_time << "&";
	content << rec.score << "&";
	content << rec.result << "&" << rec.hash << "&";
	content << rec.subcontinent.first << "&";
	content << rec.employee << "," << rec.subcontinent.second;

	trequest request(*this, "uploadsiege", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		if (str == "score_error") {
			symbols["score"] = tintegrate::generate_format(rec.score, "red");
			err << vgettext2("This save's score($score) is invalid, cannot upload!", symbols);
		...
		} else if (str == "error_max_sieges") {
			symbols["max"] = tintegrate::generate_format(game_config::max_sieges, "red");
			err << vgettext2("Had uploaded at least $max sieges today. cannot upload more!", symbols);
...
membership http_agent::do_uploadmessage(const tmessage_record& rec)
{
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		if (str == "duplicate") {
			symbols["create"] = tintegrate::generate_format(format_time_date(rec.create_time), "red");
			err << vgettext2("Had uploaded message that create at $create, cannot upload again!", symbols);
			
		} else if (str == "error_receiver") {
			symbols["create"] = tintegrate::generate_format(rec.receiver_username, "red");
			err << vgettext2("Receiver don't exist!", symbols);
...
membership http_agent::do_upgrade(int number, int coin, int score)
{
	...
	trequest request(*this, "upgrade", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(_("Upgrade"), "yellow");
		if (str == "mishero") {
			err << vgettext2("When $action, unknown hero!", symbols);
		} else if (str == "misstatus") {
			err << vgettext2("When $action, error status!", symbols);
		} else if (str == "misscore") {
			err << vgettext2("When $action, error score!", symbols);
		} 
		if (err.str().empty()) {
			member = parse_membership_result(str);
			if (member.uid < 0) {
				err << vgettext2("When $action, unknown error!", symbols);
			}
		}
		if (!err.str().empty()) {
			gui2::show_message(disp_.video(), "", err.str());
...
membership http_agent::do_member(int op, const std::vector<std::pair<int, int> >& m, int coin, int score)
{
	...
	if (ops.empty()) {
		ops.push_back(member_pair("reload", _("member^Reload")));
		ops.push_back(member_pair("insert", _("member^Insert")));
		ops.push_back(member_pair("erase", _("member^Erase")));
		ops.push_back(member_pair("resort", _("member^Resort")));
	}
	...
	std::stringstream strstr;
	for (std::vector<std::pair<int, int> >::const_iterator it = m.begin(); it != m.end(); ++ it) {
		if (it != m.begin()) {
			strstr << ", ";
		}
	...
	std::stringstream content;
	content << "do=" << ops[op].op;
	content << "&member=" << strstr.str();
	content << "&coin=" << coin;
	content << "&score=" << score;

	trequest request(*this, "member", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(ops[op].name, "yellow");
		if (str == "mishero") {
			err << vgettext2("When $action, unknown hero!", symbols);
		} else if (str == "misstatus") {
			err << vgettext2("When $action, error status!", symbols);
...
membership http_agent::do_exile(int tag, const std::vector<std::pair<int, int> >& m, int coin, int score)
{
	...
	const std::string& _do = exile_tag_str(tag);
	if (_do.empty()) {
		return member;
	}
	if (!do_prepare(true)) {
	...
	std::stringstream content;
	content << "do=" << _do;
	content << "&exile=" << strstr.str();
	content << "&coin=" << coin;
	content << "&score=" << score;

	trequest request(*this, "exile", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(_("Exile"), "yellow");
		if (str == "mishero") {
			err << vgettext2("When $action, unknown hero!", symbols);
		} else if (str == "misstatus") {
			err << vgettext2("When $action, error status!", symbols);
...
membership http_agent::do_score(int coin, int score, bool transaction)
{
	...
	trequest request(*this, "score", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(_("Score"), "yellow");
		if (str == "mishero") {
			err << vgettext2("When $action, unknown hero!", symbols);
		} else if (str == "misstatus") {
			err << vgettext2("When $action, error status!", symbols);
...
membership http_agent::upload_data(const std::map<int, std::string>& block, bool quiet, int uid)
{
	...
	std::stringstream content;
	for (std::map<int, std::string>::const_iterator it = block.begin(); it != block.end(); ++ it) {
		if (it != block.begin()) {
			content << "&";
		}
		content << block_tag_str(it->first) << "#" << it->second;
	}
	if (uid >= 0) {
		content << "&uid#" << uid;
	}

	trequest request(*this, "blockdata", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(_("Save Layout"), "yellow");
		if (str == "mishero") {
			err << vgettext2("When $action, unknown hero!", symbols);
		} else if (str == "misstatus") {
			err << vgettext2("When $action, error status!", symbols);
...
membership http_agent::insert_transaction(const std::string& number, const std::string& buyer, int request_uid, int type)
{
	...
	content << "&number=" << number;
	content << "&buyer=" << buyer;
	content << "&uid=" << request_uid;

	trequest request(*this, "transaction", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(_("transaction^Insert"), "yellow");
		if (str == "number_error") {
			err << vgettext2("When $action, error number!", symbols);
		} else if (str == "buyer_error") {
			err << vgettext2("When $action, error buyer!", symbols);
		} else if (str == "number_duplicate") {
			err << vgettext2("When $action, duplicate number!", symbols);
		} else {
			member = parse_membership_result(str);
			if (member.uid < 0) {
				err << vgettext2("When $action, unknown error!", symbols);
...
membership http_agent::do_associate(const std::string& username, int tag, bool check_network)
{
	...
	const std::string& _do = associate_tag_str(tag);
	if (_do.empty()) {
		return member;
	}
	if (!do_prepare(check_network)) {
		return member;
	}

	std::stringstream content;
	content << "do=" << _do << "&username=" << username;

	trequest request(*this, "associate", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(_("Associate"), "yellow");
		if (str == "username_cannot_myself") {
			err << _("Username cannot be myself!");
		} else if (str == "misstatus") {
			err << vgettext2("When $action, error status!", symbols);
		} else if (str == "misscore") {
			err << vgettext2("When $action, error score!", symbols);
...
membership http_agent::do_employee_insert(hero_map& heros)
{
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		if (str == "id_absent") {
			err << vgettext2("When $action, id_absent!", symbols);
		} else if (str == "misstatus") {
			err << vgettext2("When $action, error status!", symbols);
		} else if (str == "misscore") {
			err << vgettext2("When $action, error score!", symbols);
...
membership http_agent::do_employee_common(int tag, const std::set<int>& number, int coin, int score, std::map<int, temployee>* employees)
{
	...
	const std::string& _do = employee_tag_str(tag);
	if (_do.empty() || number.empty()) {
		return member;
	}
	if (!do_prepare(true)) {
		return member;
	...
	content << "do=" << _do << "&param=";
	bool first = true;
	for (std::set<int>::const_iterator it = number.begin(); it != number.end(); ++ it) {
		if (!first) {
			content << ",";
		} else {
			first = false;
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		symbols["action"] = tintegrate::generate_format(_("Employee"), "yellow");
		if (str == "uid_error") {
			err << vgettext2("uid_error", symbols);
		} else if (str == "id_absent") {
			err << vgettext2("id_absent", symbols);
		} else if (str == "lock_status_error") {
			err << vgettext2("lock_status_error", symbols);
		} else if (str == "level_error") {
			err << vgettext2("level_error", symbols);
		} 
		if (!err.str().empty()) {
			gui2::show_message(disp_.video(), "", err.str());
			return member;
		...
		std::string membership_size_str, listemployee_size_str;
		membership_size_str.assign(content, 8);
		char* endptr;
		int membership_size = (int)strtol(membership_size_str.c_str(), &endptr, 16);
		if (content_length <= 8 + membership_size) {
			return member;
		}
		listemployee_size_str.assign(content + 8 + membership_size, 8);
		int listemployee_size = (int)strtol(listemployee_size_str.c_str(), &endptr, 16);
		if (content_length != 8 + membership_size + 8 + listemployee_size) {
			return member;
		}

		std::string membership_str, listemployee_str;
		membership_str.assign(content + 8, membership_size);
		listemployee_str.assign(content + 8 + membership_size + 8, listemployee_size);

		member = parse_membership_result(membership_str, false);
		if (member.uid >= 0 && employees) {
			*employees = parse_employeelist_str_2_map(listemployee_str);
		}
	}
	return member;
}
...
membership http_agent::do_signin(int tag)
{
	...
	const std::string& _do = signin_tag_str(tag);
	if (_do.empty()) {
		return member;
	}
	if (!do_prepare(true)) {
	...
	trequest request(*this, "signin", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(_("Sign in"), "yellow");
		if (str == "uid_error") {
			err << vgettext2("uid_error", symbols);
		} else if (str == "id_absent") {
			err << vgettext2("id_absent", symbols);
		...
		member = parse_membership_result(str, false);
	}
	return member;
}
...
std::pair<tsubcontinent_record, membership> http_agent::do_subcontinent(int tag, const std::string& id, int cityno)
{
	...
	const std::string& _do = subcontinent_tag_str(tag);
	if (_do.empty()) {
		return std::make_pair(rec, member);
	}
	if (!do_prepare(true)) {
	...
	content << "do=" << _do << "&";
	content << "id=" << id << "&";
	content << "cityno=" << cityno;

	trequest request(*this, "subcontinent", content.str());
	network_send_dialog(disp_, "", request.buf, request.size, sock_);
	...
	if (content_start != -1 && content_start < (int)buf.size()) {
		...
		std::stringstream err;
		utils::string_map symbols;
		symbols["action"] = tintegrate::generate_format(_("Subcontinent"), "yellow");
		if (str == "error_do") {
			err << vgettext2("error do", symbols);
		} else if (str == "error_param") {
			err << vgettext2("error param", symbols);
		} 
		if (!err.str().empty()) {
			gui2::show_message(disp_.video(), "", err.str());
			return std::make_pair(rec, member);
		...
		rec = parse_subcontinent_result(vstr[0]);
		member = parse_membership_result(vstr[1], true);
	}
	return std::make_pair(rec, member);
}
...
membership affirm_ally(display& disp, hero_map& heros, const std::string& username, bool dialog, bool ok)
{
	...
	if (dialog) {
		...
		message = vgettext2("$username wants to ally, do you agree?", symbols);
...
membership affirm_terminate(display& disp, hero_map& heros, const std::string& username, bool dialog)
{
	...
	if (dialog) {
		...
		message = vgettext2("Notice, $username declares to terminate with you!", symbols);
...
bool employee_employ(display& disp, hero_map& heros, hero& h, int score, std::map<int, temployee>* employees)
{
	...
	if (sum_score(group.coin(), group.score()) < score) {
		...
		message = vgettext2("Repertory is less than $cost score!", symbols);
		gui2::show_message(disp.video(), "", message);
		return false;
	}
	...
	message = vgettext2("Do you want to spend $cost score to employ $name?", symbols);
	int	coin_income = 0;
...
std::string calculate_res_checksum(display& disp, const config& game_config)
{
	...
	// requrie check file: data.bin, hero.dat, all campaign.bin
	...
	strstr << "," << checksum;
	// all campaign.bin
	BOOST_FOREACH (const config &i, game_config.child_range("campaign")) {
		std::string name = i["id"].str() + ".bin";
		if (!wml_checksum_from_file(game_config::path + "/xwml/campaigns/" + name, &bin_nfiles, &bin_sum_size, &bin_modified)) {
			bin_nfiles = bin_sum_size = bin_modified = 0;
		}
		strstr << "," << bin_nfiles << "," << bin_sum_size << "," << bin_modified;
	}
	...
	if (game_config::bbs_server.host.find("freeors.com") != std::string::npos) {
		ret = "79a67f72b4";
	} else if (game_config::bbs_server.host.find("sanguohero.com") != std::string::npos) {
		ret = "03b7cf9b67";

	} else if (game_config::bbs_server.host.find("localhost") != std::string::npos) {
