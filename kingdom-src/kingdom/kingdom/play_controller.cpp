...
void show_tick_infromation(game_display& disp, const unit_map& units, const play_controller& controller)
{
	...
	for (unit_map::const_iterator it = units.begin(); it != units.end(); ++ it) {
		const unit& u = *dynamic_cast<unit*>(&*it);
		if (!u.consider_ticks()) {
			// continue;
		}
...
*/
			...
			if (gamestate_.classification().campaign.find("subcontinent_") == 0) {
				...
				for (city_map::const_iterator it = citys.begin(); it != citys.end(); ++ it) {
					...
					if (loc.x < lt.x || loc.x > rb.x || loc.y < lt.y || loc.y > rb.y) {
						...
						err << vgettext2("Map is for subcontinent. Find improper city location: $name($x, $y).", symbols);
						break;
					}
				}
				if (!err.str().empty()) {
					...
					err << vgettext2("Location.x of city must be in [$x1, $x2], y in [$y1, $y2]!", symbols);
	...
	const config &theme_cfg = gui2::get_theme(level_["theme"]);
...
void play_controller::rpg_exchange(const std::vector<size_t>& human_p, size_t ai_p) 
{
	...
	game_events::show_hero_message(rpg::h, NULL, vgettext2("You exchanged $ai using $human.", symbols), game_events::INCIDENT_INVALID);
...
void play_controller::rpg_independence(bool replaying) 
{
	...
	game_events::show_hero_message(rpg::h, rpg_city, vgettext2("$first declared independence.", symbols), game_events::INCIDENT_INDEPENDENCE);

	if (from_team.defeat_vote()) {
		aggressing = NULL;

		symbols["first"] = tintegrate::generate_format(from_team.name(), tintegrate::hero_color);
		game_events::show_hero_message(&heros_[hero::number_scout], NULL, vgettext2("$first is defeated.", symbols), game_events::INCIDENT_DEFEAT);
	}
	...
	if (from_leader_unit && aggressing) {
		...
		if (same) {
			...
			if (rpg_city->get_location() == from_leader_unit->get_location()) {
				...
				// field troop
				units_.erase2(from_leader_unit);
			}
	...
	for (std::vector<artifical*>::iterator it = holden_cities.begin(); it != holden_cities.end(); ++ it) {
		...
		if (independenced && city != rpg_city) {
			...
			game_events::show_hero_message(&heros_[hero::number_scout], city, vgettext2("$first declared belonging to $second.", symbols), game_events::INCIDENT_INVALID);
		}
	}
...
void play_controller::technology_tree()
{
	...
	{
		items.push_back(gui2::tsystem::titem(_("Assemble Treasure")));
		items.push_back(gui2::tsystem::titem(_("Appoint Noble")));
		items.push_back(gui2::tsystem::titem(_("Technology tree")));

		if (tent::mode == mode_tag::LAYOUT) {
			items.push_back(gui2::tsystem::titem(_("Stratagem")));
		}
...
void play_controller::start_pass_scenario_anim(LEVEL_RESULT result) const
{
	...
	if (const animation* tpl = anim2::anim(anim2::PASS_SCENARIO)) {
		animation& anim = gui_->insert_pass_scenario_anim(*tpl);

		std::stringstream strstr;

		anim.replace_static_text("diffleft", "__difficulty", _("scenario^Difficulty"));
		strstr.str("");
		strstr << "~CROP(0, 0, " << (48 * difficulty_level_) << ", 48)";
		anim.replace_image_mod("diffright", "__diffmod", strstr.str());
		anim.replace_progressive("diffright", "x", "5555", str_cast(48 * difficulty_level_ / 2));

		anim.replace_static_text("turnleft", "__turns", _("scenario^Turns"));
		anim.replace_static_text("turnright", "__turns_count", str_cast(turn()));

		if (tent::tower_mode()) {
			anim.replace_static_text("line3left", "__line3title", _("scenario^Capture fort"));
			anim.replace_image_name("line3right", "__line3image", capture? "misc/ok.png": "misc/delete.png");

		} else {
			anim.replace_static_text("line3left", "__line3title", "");
			anim.replace_image_name("line3right", "__line3image", "");
		}
		anim.replace_static_text("line4left", "__line4title", _("scenario^Perfect turns"));
		anim.replace_static_text("line4right", "__line4text", str_cast(t.perfect_turns_));


		anim.replace_static_text("scoreright", "__score", str_cast(score));
		anim.replace_static_text("coinright", "__coin", str_cast(coin));
...
void play_controller::upload()
{
	...
	if (tent::mode != mode_tag::SIEGE) {
		...
		if (rec.attacker == rec.defender) {
			gui2::show_transient_message(gui_->video(), "", _("There is in test mode, cannot upload!"));
			return;
		}
		...
		if (now < gamestate_.classification().create || now - gamestate_.classification().create > siege_upload_threshold) {
			...
			strstr << vgettext2("Save is more than $threshold hour, cannot upload!", symbols);
	...
	if (m.uid >= 0) {
		...
		if (employee.first != HEROS_INVALID_NUMBER) {
			...
			strstr << vgettext2("$username lose $name!", symbols) << "\n\n";
		}
		strstr << _("Upload successfully!");
		gui2::show_message(gui_->video(), "", strstr.str());
	}
}
...
void play_controller::final_battle(int side_num, int human_capital, int ai_capital)
{
	...
	if (final_capital().first) {
		...
		symbols["second"] = final_capital().second? final_capital().second->master().name(): _("Void");
		gui2::show_transient_message(gui_->video(), _("final battle"), vgettext2("human capital: $first\n\n\n\nai capital: $second", symbols));
		return;
	}
...
void play_controller::system()
{
	...
	{
		if (tent::mode != mode_tag::LAYOUT) {
			items.push_back(gui2::tsystem::titem(_("Save Game"), !disable_exit && !is_replaying()));
			rets.push_back(_SAVE);

			items.push_back(gui2::tsystem::titem(_("Save Replay"), !disable_exit && !is_replaying()));
			rets.push_back(_SAVEREPLAY);

			items.push_back(gui2::tsystem::titem(_("Load Game"), !disable_exit));
			rets.push_back(_LOAD);
		} else {
			str = tintegrate::generate_format(_("Save Layout"), "blue");
			tgroup& g = runtime_groups::get(t.leader()->number_);
		...
		if (!game_config::tiny_gui) {
			items.push_back(gui2::tsystem::titem(_("Help")));
			rets.push_back(_HELP);
		}
		
		items.push_back(gui2::tsystem::titem(_("Quit"), !disable_exit));
	...
	if (rets[retval] == _SAVE) {
		...
		if (gui_->in_theme()) {
			...
			if (map_str_dirty) {
				symbols["name"] = tintegrate::generate_format(_("Map"), "red");
			} else if (stratagem_dirty) {
				symbols["name"] = tintegrate::generate_format(_("Stratagem"), "red");
			} else if (layout_str_dirty) {
				symbols["name"] = tintegrate::generate_format(_("group^Layout"), "red");
			}
			if (!symbols.empty()) {
				strstr << vgettext2("$name is dirty, but not save.", symbols) << " ";
			}

			strstr << _("Do you really want to quit?");
...
void play_controller::bomb()
{
	...
	if (current_team.bomb_turns() >= game_config::max_bomb_turns) {
		...
		message = vgettext2("Do you want to spend $cost gold to purchase move?", symbols);
		...
		message = vgettext2("If have $cost gold, you can purchase move.", symbols);
...
{
	unit* u_itor = find_visible_unit(mouse_handler_.get_selected_hex(), teams_[gui_->viewing_team()]);
	if (u_itor) {
...
void play_controller::save_game()
{
	if(save_blocker::try_block()) {
		...
		save.save_game_interactive(gui_->video(), "", true);
	} else {
...
{
	gui2::tbook2 dlg(*gui_, &map_, heros_, game_config_, "help");
	dlg.show(gui_->video());
}
		...
		if (t.ing_technology()) {
			...
			if (city && !tent::tower_mode()) {
				game_events::show_hero_message(&heros_[hero::number_civilian], city, 
					vgettext2("$side begin to research $begin!", symbols), game_events::INCIDENT_TECHNOLOGY);
			}
		}
	}
...
void play_controller::more_card(team& current_team, int turn)
{
	...
	if (current_team.add_card(add.number_, true)) {
		...
		game_events::show_hero_message(&heros_[hero::number_scout], NULL, vgettext2("Get card: $first.", symbols), game_events::INCIDENT_CARD);
	}
}
...
void play_controller::execute_command2(int command, const std::string& sparam)
{
	...
	case HOTKEY_REDO:
		redo();
		break;
	case tgame_theme::HOTKEY_INTERIOR:
		interior();
		break;
	case tgame_theme::HOTKEY_TECHNOLOGY_TREE:
		technology_tree();
		break;
	case tgame_theme::HOTKEY_UPLOAD:
		upload();
		break;
	case tgame_theme::HOTKEY_FINAL_BATTLE:
		final_battle(-1, -1, -1);
		break;
	case tgame_theme::HOTKEY_EMPLOY:
		employ();
		break;
	case tgame_theme::HOTKEY_LIST:
		list();
		break;
	case HOTKEY_SYSTEM:
		system();
	...
	case tgame_theme::HOTKEY_TACTIC2:
		remove_active_tactic(command - tgame_theme::HOTKEY_TACTIC0);
		break;
	case tgame_theme::HOTKEY_BOMB:
		bomb();
		break;
	case tgame_theme::HOTKEY_RPG:
		show_rpg();
		break;
	case tgame_theme::HOTKEY_RPG_DETAIL:
		rpg_detail();
		break;
	case tgame_theme::HOTKEY_RPG_TREASURE:
		assemble_treasure();
		break;
	case tgame_theme::HOTKEY_RPG_EXCHANGE:
		rpg_exchange(empty_vector_size_t, -1);
		break;
	case tgame_theme::HOTKEY_RPG_INDEPENDENCE:
		rpg_independence();
		break;
	case tgame_theme::HOTKEY_UNIT_DETAIL:
		unit_detail();
		break;
	default:
...
bool play_controller::can_execute_command(int command, const std::string& sparam) const
{
	using namespace gui2;
...
void play_controller::set_zoom(int new_zoom)
{
	units_.set_zoom(new_zoom);
}
...
void play_controller::process_keyup_event(const SDL_Event& event) {
	...
	if(event.key.keysym.sym >= '1' && event.key.keysym.sym <= '7') {
		...
		if(new_path_turns != mouse_handler_.get_path_turns()) {
			...
			unit* u = mouse_handler_.selected_unit();
...
void play_controller::post_mouse_press(const SDL_MouseButtonEvent& /*button*/)
{
...
bool play_controller::in_context_menu(const std::string& id) const
{
	...
	if (command != tgame_theme::HOTKEY_LIST && command != HOTKEY_SYSTEM && !replaying_ && !current_team.is_human()) {
		return false;
	}
	if (linger_) {
		if (command == tgame_theme::HOTKEY_LIST || command == HOTKEY_SYSTEM) {
			return true;
		} else {
			return command == tgame_theme::HOTKEY_UPLOAD && !replaying_;
		}
	}
	if (replaying_ && (command == tgame_theme::HOTKEY_TECHNOLOGY_TREE || command == tgame_theme::HOTKEY_FINAL_BATTLE)) {
	...
	case tgame_theme::HOTKEY_BUILD_M:
	...
	case tgame_theme::HOTKEY_BUILD:
		...
		if (!itor || itor->is_artifical() || !itor->human()) {
			return false;
		}
		if (command == tgame_theme::HOTKEY_BUILD && !in_build_menu(item.second)) {
			return false;
		}
		if (tent::tower_mode()) {
			return true;
	...
	case tgame_theme::HOTKEY_GUARD:
		...
		if (itor && !itor->is_artifical() && (itor->side() == player_number_) && itor->human()) {
			return true;
		}
		break;
	...
	case tgame_theme::HOTKEY_EXTRACT:
		...
		if (itor && !itor->is_artifical() && (itor->side() == player_number_)) {
			return true;
		}
		break;
	...
	case tgame_theme::HOTKEY_ADVANCE:
		...
		if (tent::mode != mode_tag::LAYOUT || !itor || itor->side() != player_number_) {
			return false;
		}
	...
	case tgame_theme::HOTKEY_INTERIOR:
		...
		return !events::commands_disabled && !itor;
	...
	case tgame_theme::HOTKEY_EMPLOY:
		...
		return replaying_ || (!events::commands_disabled && !itor);
	...
	case tgame_theme::HOTKEY_FINAL_BATTLE:
		...
		return !events::commands_disabled && (teams_.size() >= 3) && !itor;
	...
	case HOTKEY_SYSTEM:
		...
		return replaying_ || (!events::commands_disabled && !itor && (has_network_player() || current_team.is_human()));
...
bool play_controller::actived_context_menu2(const std::string& id, const map_location& loc) const
{
	...
	case tgame_theme::HOTKEY_EXTRACT:
		if (!itor || itor->is_artifical() || (itor->side() != player_number_) || !itor->second().valid()) {
			return false;
		}
		break;

	case tgame_theme::HOTKEY_ADVANCE:
		if (!itor || itor->side() != player_number_) {
			return false;
		}
		return itor->type()->can_advance();

	case tgame_theme::HOTKEY_DEMOLISH:
		if (!itor || itor->side() != player_number_ || unit_is_city(&*itor)) {
			return false;
		}
			...
			if (rpg::stratum == hero_stratum_mayor) {
				if (art->type() == unit_types.find_market()) {
					...
					for (int i = 0; i != adjacent_size; ++i) {
						unit* another = units_.find_unit(tiles[i], true);
						if (another && another->is_city()) {
...
void play_controller::prepare_show_menu(gui2::tbutton& widget, const std::string& id, int width, int height) const
{
	...
	if (command == tgame_theme::HOTKEY_BUILD) {
	...
	} else if (command == tgame_theme::HOTKEY_BUILD_M) {
		if (tent::tower_mode()) {
			strstr.str("");
	...
	} else if (command == tgame_theme::HOTKEY_ABOLISH) {
		strstr.str("");
		strstr << "buttons/" << item.first << ".png";
...
void play_controller::process_oos2(const std::string& msg) const
{
	...
	strstr << msg;
	strstr << "\n\n";
	strstr << vgettext2("Sorry, run into program BUG. Program has save scene to $save. Hope you to upload it to sever, so that developer can solve it.", symbols);
...
void play_controller::calculate_commoner_gotos(team& current_team, std::vector<std::pair<unit*, int> >& gotos)
{
	...
	for (size_t i = 0; i < troops_vsize; i ++) {
		...
		if (u.is_commoner()) {
			if (!u.transport()) {
				...
				const unit& fort = *units_.find_unit(loc, false);
	...
	for (std::map<map_location, int>::iterator it = forts.begin(); it != forts.end(); ++ it) {
		const unit& fort = *units_.find_unit(it->first, false);
...
void play_controller::commoner_back(artifical* entered_city, artifical* belong_city, const map_location& src_loc)
{
	...
	units_.insert(src_loc, &back_commoner);
	...
	while (++ times <= 2 && !curr->empty()) {
		for (std::vector<std::pair<unit*, int> >::iterator g = curr->begin(); g != curr->end(); ++g) {
			...
			if (g->second < 0 && arrived_at == src_loc) {
				const unit& u = *units_.find_unit(src_loc, true);
				if (u.movement_left()) {
	...
	if (from != to) {
		...
		if (!unit_ptr->is_path_along_road()) {
			...
			while (!dst_must_reachable) {
				...
				if (!units_.valid2(to, true)) {
					unit* base = units_.find_unit(to, false);
					if (!base || base->can_stand(*unit_ptr)) {
						break;
					}
		...
		// do_execute
		move_spectator.set_unit(units_.find_unit(from, true));
		...
		if (steps.size() >= 2) {
			if (pair.second >= 0) {
				...
				if (!move_spectator.get_unit() || move_spectator.get_unit()->get_location() != from) {
			...
			if (!move_spectator.get_unit()) {
				// mover is died!
				unit_location_ = map_location();
			} else if (move_spectator.get_ambusher() || !move_spectator.get_seen_enemies().empty() || !move_spectator.get_seen_friends().empty() ) {
				unit_location_ = move_spectator.get_unit()->get_location();
				gamestate_changed = true;
			} else if (move_spectator.get_unit()){
				unit_location_ = move_spectator.get_unit()->get_location();
		...
		if (gamestate_changed) {
			unit* un = units_.find_unit(unit_location_, true);
			if (unit_location_ != from && un && un->movement_left() > 0) {
...
bool play_controller::do_ally(bool alignment, int my_side, int to_ally_side, int emissary_number, int target_side, int strategy_index)
{
	...
	if (s.type_ != strategy::DEFEND || !target_team.is_human() || !to_ally_team.is_enemy(target_side)) {
		...
		if (!mrs[0].enemy_cities.empty() && mrs[0].enemy_cities[0]->side() == my_side) {
			// game_events::show_hero_message(&heros[hero::number_scout], NULL, vgettext2("$first wants to ally with $second, but $second refuse.", symbols), game_events::INCIDENT_ALLY);
			return false;
		}
	}
	...
	if (s.type_ == strategy::AGGRESS) {
		message = vgettext2("In order to aggress upon $city, $first and $second concluded treaty of alliance.", symbols);
	} else if (s.type_ == strategy::DEFEND) {
		message = vgettext2("In order to defend $city, $first and $second concluded treaty of alliance.", symbols);
	} else {
		...
		for (unit_type_data::unit_type_map::const_iterator it = types.begin(); it != types.end(); ++ it) {
			...
			if (hero::is_commoner(ut.master())) {
				if (ut.movement() + 6 > (int)shortest_road) {
					...
					err << vgettext2("Map error. Too short road from $from to $to, at least $shortest!", symbols);
					VALIDATE(false, err.str());
				}
			}
		}
...
bool play_controller::click_report(gui2::twidget* widget)
{
