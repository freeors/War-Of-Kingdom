...
class scoped_player : public scoped_wml_variable
{
public:
	scoped_player(const std::string& var_name, const unsigned int hero, const int side)
		: scoped_wml_variable(var_name), hero_(hero), side_(side), shroud_(true), fog_(true) {}
	void activate();
	void set_shroud(bool shroud, bool fog);
private:
	unsigned int hero_;
	int side_;
	bool shroud_, fog_;
...
{
public:
	scoped_xy_unit(const std::string& var_name, const int x, const int y, const unit_map& umap, const artifical* city = NULL)
		: scoped_wml_variable(var_name), x_(x), y_(y), umap_(umap), city_(city) {}
	void activate();
private:
	const int x_, y_;
	const artifical* city_;
	const unit_map& umap_;
};

class scoped_hero : public scoped_wml_variable
{
public:
	scoped_hero(const std::string& var_name, int number, const hero_map& hmap)
		: scoped_wml_variable(var_name), number_(number), hmap_(hmap) {}
	void activate();
private:
	const int number_;
	const hero_map& hmap_;
};
