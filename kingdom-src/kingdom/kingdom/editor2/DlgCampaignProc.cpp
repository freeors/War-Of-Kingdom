...
void tside::update_to_ui_side_edit(HWND hdlgP, bool partial)
{
	...
	if (!partial) {
		...
		strstr.str("");
		strstr << dgettext_2_ansi("kingdom-lib", "Ally") << "(" << editor_config::ListView_GetCheckedCount(hctl) << ")";
...
BOOL On_DlgCampaignMainInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	Button_SetText(GetDlgItem(hdlgP, IDC_CHK_CAMPMAIN_SUBCONTINENT), dgettext_2_ansi("kingdom-lib", "Subcontinent"));
...
BOOL On_DlgSideEditInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_NAVIGATION), dgettext_2_ansi("kingdom-lib", "Navigation civilization"));
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_ALLY), dgettext_2_ansi("kingdom-lib", "Ally"));
	...
	lvc.fmt = LVCFMT_LEFT;
	lvc.cx = 60;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Arms"));
	lvc.pszText = text;
	lvc.cchTextMax = 0;
	lvc.iSubItem = 0;
	ListView_InsertColumn(hctl, 0, &lvc);
	...
	lvc.cx = 50;
	lvc.iSubItem = 1;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Level"));
	lvc.pszText = text;
	...
	lvc.cx = 60;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Soldiers"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
	...
	lvc.cx = 70;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Traits"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
	...
	lvc.cx = 50;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Character"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
	...
	lvc.cx = 60;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Mayor"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
	...
	lvc.cx = 54;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "economy area"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
...
BOOL On_DlgFeature2EditInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_ARMS), dgettext_2_ansi("kingdom-lib", "Arms"));
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_LEVEL), dgettext_2_ansi("kingdom-lib", "Level"));
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_FEATURE), dgettext_2_ansi("wesnoth-hero", "feature"));
...
BOOL On_DlgCityEditInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_MAYOR), dgettext_2_ansi("kingdom-lib", "Mayor"));
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_CHARACTER), dgettext_2_ansi("kingdom-lib", "Character"));
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_SOLDIERS), dgettext_2_ansi("kingdom-lib", "Soldiers"));
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_TRAITS), dgettext_2_ansi("kingdom-lib", "Traits"));
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_EA), dgettext_2_ansi("kingdom-lib", "economy area"));
	...
	if (lpNMHdr->code == NM_CLICK) {
		...
		if ((lpNMHdr->idFrom == IDC_LV_SIDEEDIT_BUILD 
			...
			if (lpNMHdr->idFrom == IDC_LV_SIDEEDIT_BUILD) {
				...
				strstr << dgettext_2_ansi("kingdom-lib", "Ally") << "(" << editor_config::ListView_GetCheckedCount(lpNMHdr->hwndFrom) << ")";
				Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_ALLY), strstr.str().c_str());
			} 
		}
...
BOOL On_DlgCampaignScenarioInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	lvc.cx = 70;
	lvc.iSubItem = 1;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "City"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, 1, &lvc);
	...
	lvc.cx = 60;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Controller"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);

	lvc.cx = 40;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Fog"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);

	lvc.cx = 54;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Card"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
	...
	lvc.cx = 60;
	lvc.iSubItem = column;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Navigation civilization"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
...
BOOL On_DlgRoadInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_CITY1), dgettext_2_ansi("kingdom-lib", "City"));
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_CITY2), dgettext_2_ansi("kingdom-lib", "City"));
...
void campaign_refresh(HWND hdlgP)
{
	...
	if (tmain::catalog_map.empty()) {
		...
		tmain::catalog_map.push_back(std::make_pair("tutorial", _("Tutorial")));
	}
...
void fill_hero_state_row(const tscenario& scenario, HWND hctl, hero& h, const tscenario::hero_state& state, int iItem)
{
	...
	for (std::map<int, tscenario::hero_state::tstate>::const_iterator it = state.states_.begin(); it != state.states_.end(); ++ it) {
		...
		if (it->first < (int)scenario.side_.size()) {
			...
			strstr << "  ";
			strstr << _("Status") << ": " << tscenario::state_name(s.state);
			continue;
		}
