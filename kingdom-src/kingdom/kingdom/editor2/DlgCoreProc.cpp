...
void update_to_ui_special(HWND hdlgP, int flag)
{
	...
	strstr.str("");
	strstr << dgettext_2_ansi("kingdom-lib", "Technology") << "(" << technology->size() << ")";
...
void tcore::refresh_decree(HWND hdlgP)
{
	...
	for (std::map<int, tdecree>::const_iterator it = decrees.begin(); it != decrees.end(); ++ it) {
		...
		if (t.front()) {
			strcpy(text, "Front");
			strstr << dgettext("kingdom-lib", text);
		}
		if (t.min_level()) {
			if (!strstr.str().empty()) {
				strstr << "  ";
...
void tcore::refresh_technology(HWND hdlgP)
{
	...
	strstr.str("");
	strstr << dgettext_2_ansi("kingdom-lib", "Technology tree") << "(" << unit_types.technologies().size() << ")";
	strcpy(text, strstr.str().c_str());
	// 这里一定要设TVIF_CHILDREN, 否则接下折叠后将判断出其cChildren为0, 再不能展开
...
void tcore::switch_section(HWND hdlgP, int to, bool init)
{
	...
	if (name_map.empty()) {
		...
		name_map[TERRAIN] = dgettext_2_ansi("kingdom-lib", "Terrain");
		...
		strstr << dgettext("kingdom-lib", text) << "(" << _("Read only") << ")";
		...
		name_map[FACTION] = dgettext_2_ansi("kingdom-lib", "Faction");
...
{
	std::stringstream strstr;
	strstr << dgettext_2_ansi("kingdom-lib", "Technology") << "--";
