...
void tunit_type::update_to_ui_utype_edit_mtype(HWND hdlgP) const
{
	...
	for (std::vector<std::string>::const_iterator it = attack_type_vstr.begin(); it != attack_type_vstr.end(); ++ it) {
		...
		lvi.iSubItem = 0;
		strstr.str("");
		strstr << utf8_2_ansi(dsgettext("kingdom-lib", it->c_str()));
		strcpy(text, strstr.str().c_str());
		lvi.pszText = text;
		lvi.lParam = (LPARAM)0;
		ListView_InsertItem(hctl, &lvi);
...
void tunit_type::update_to_ui_utype_edit_type(HWND hdlgP) const
{
	std::stringstream strstr;
	if (type() == TYPE_TROOP) {
		strstr << dgettext_2_ansi("kingdom-lib", "Movement") << ": " << movement_;
		strstr << "(";
		if (max_movement_ != -1) {
		...
		strstr << ")\r\n";
		strstr << dgettext_2_ansi("kingdom-lib", "Character") << ": ";
		...
		if (guard_ != NO_GUARD) {
			strstr << utf8_2_ansi(dsgettext("kingdom-lib", attacks_[guard_].id_.c_str()));
		}
		if (master_ == hero::number_market || master_ == hero::number_technology || master_ == hero::number_tactic) {
			strstr << "\r\n";
...
void tunit_type::tattack::update_to_ui_utype_edit(HWND hdlgP, int index) const
{
	...
	lvi.iSubItem = column ++;
	strstr.str("");
	strstr << utf8_2_ansi(dsgettext("kingdom-lib", id_.c_str())) << "(" << id_ << ")";
	strcpy(text, strstr.str().c_str());
	lvi.pszText = text;
	lvi.lParam = (LPARAM)0;
	if (lvi.iItem != count) {
	...
	lvi.iSubItem = column ++;
	strstr.str("");
	strstr << utf8_2_ansi(dsgettext("kingdom-lib", type_.c_str()));
	strcpy(text, strstr.str().c_str());
	lvi.pszText = text;
	ListView_SetItem(hctl, &lvi);
	...
	if (range_ < 0 || range_ >= (int)attack_range_vstr.size()) {
		...
		strstr << utf8_2_ansi(dsgettext("kingdom-lib", attack_range_vstr[range_].c_str()));
	}
	strcpy(text, strstr.str().c_str());
	lvi.pszText = text;
	ListView_SetItem(hctl, &lvi);
...
BOOL On_DlgUTypeEditInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	set_language_text(hdlgP, IDC_STATIC_COST, "kingdom-lib", "Cost");
	set_language_text(hdlgP, IDC_STATIC_GOLD, "kingdom-lib", "Gold");
	...
	set_language_text(hdlgP, IDC_STATIC_ARMS, "kingdom-lib", "Arms");
	set_language_text(hdlgP, IDC_STATIC_RACE, "kingdom-lib", "Race");
	...
	strstr.str("");
	strstr << "Resistance";
	strcpy(text, utf8_2_ansi(dsgettext("kingdom-lib", strstr.str().c_str())));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, 1, &lvc);
	...
	strstr.str("");
	strstr << "Character";
	strcpy(text, utf8_2_ansi(_(strstr.str().c_str())));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
...
BOOL On_DlgAttackEditInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	// textdomain
	Edit_SetText(GetDlgItem(hdlgP, IDC_ET_ATTACKEDIT_TEXTDOMAIN), "kingdom-lib");
	// id
	...
	Edit_SetText(GetDlgItem(hdlgP, IDC_ET_ATTACKEDIT_NAME), utf8_2_ansi(dsgettext("kingdom-lib", attack.id_.c_str())));
	// icon
	...
	for (std::vector<std::string>::const_iterator it = attack_type_vstr.begin(); it != attack_type_vstr.end(); ++ it, index ++) {
		ComboBox_AddString(hctl, utf8_2_ansi(dsgettext("kingdom-lib", it->c_str())));
...
BOOL On_DlgMTypeEditInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	set_language_text(hdlgP, IDC_STATIC_RESISTANCE, "kingdom-lib", "Resistance");
	...
	lvc.fmt = LVCFMT_LEFT;
	lvc.cx = 80;
	strcpy(text, dgettext_2_ansi("rose-lib", "Terrain"));
	lvc.pszText = text;
	lvc.cchTextMax = 0;
	lvc.iSubItem = 0;
	ListView_InsertColumn(hctl, 0, &lvc);
...
BOOL On_DlgUTypeTroopInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	set_language_text(hdlgP, IDC_STATIC_MOVEMENT, "kingdom-lib", "Movement");
	...
	set_language_text(hdlgP, IDC_STATIC_CHARACTER, "kingdom-lib", "Character");
...
BOOL On_DlgUTypeArtificalInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	for (std::vector<tunit_type::tattack>::const_iterator it = ns::utype.attacks_.begin(); it != ns::utype.attacks_.end(); ++ it, index ++) {
		ComboBox_AddString(hctl, utf8_2_ansi(dgettext("kingdom-lib", it->id_.c_str())));
...
BOOL On_DlgUType2EditInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	lvc.cx = 50;
	lvc.iSubItem = 1;
	strcpy(text, dgettext_2_ansi("kingdom-lib", "Resistance"));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, 1, &lvc);
...
BOOL On_DlgVisual2InitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	if (ns::type == IDM_UTYPE_GENERAL) {
		...
		strstr << dgettext_2_ansi("kingdom-lib", "Resistance");
	...
	if (ns::type == IDM_UTYPE_GENERAL) {
		...
		lvc.cx = 50;
		lvc.iSubItem = index;
		strcpy(text, dgettext_2_ansi("kingdom-lib", "Movement"));
		lvc.pszText = text;
		ListView_InsertColumn(hctl, index ++, &lvc);
		...
		lvc.cx = 50;
		lvc.iSubItem = index;
		strcpy(text, dgettext_2_ansi("kingdom-lib", "Cost"));
		lvc.pszText = text;
		ListView_InsertColumn(hctl, index ++, &lvc);
		...
		lvc.cx = 50;
		lvc.iSubItem = index;
		strcpy(text, dgettext_2_ansi("kingdom-lib", "Income"));
		lvc.pszText = text;
		ListView_InsertColumn(hctl, index ++, &lvc);
		...
		for (std::vector<std::string>::const_iterator it = attack_type_vstr.begin(); it != attack_type_vstr.end(); ++ it) {
			...
			lvc.iSubItem = index;
			strstr.str("");
			strstr << utf8_2_ansi(dgettext("kingdom-lib", it->c_str()));
			strcpy(text, strstr.str().c_str());
			lvc.pszText = text;
			ListView_InsertColumn(hctl, index ++, &lvc);
		}
	...
	for (std::vector<std::pair<std::string, std::vector<std::string> > >::const_iterator it = ns::core.tv_tree_.begin(); it != ns::core.tv_tree_.end(); ++ it, iItem ++) {
		...
		if (ns::type == IDM_UTYPE_GENERAL) {
			...
			lvi.iSubItem = index ++;
			strstr.str("");
			strstr << utf8_2_ansi(dgettext("kingdom-lib", attack.id().c_str())) << "(" << attack.id() << ")";
			strcpy(text, strstr.str().c_str());
			lvi.pszText = text;
			ListView_SetItem(hctl, &lvi);
			...
			lvi.iSubItem = index ++;
			strstr.str("");
			strstr << utf8_2_ansi(dgettext("kingdom-lib", attack.type().c_str()));
			strcpy(text, strstr.str().c_str());
			lvi.pszText = text;
			ListView_SetItem(hctl, &lvi);
			...
			if (range < 0 || range >= (int)attack_range_vstr.size()) {
				...
				strstr << utf8_2_ansi(dgettext("kingdom-lib", attack_range_vstr[range].c_str()));
			}
			strcpy(text, strstr.str().c_str());
			lvi.pszText = text;
			ListView_SetItem(hctl, &lvi);
