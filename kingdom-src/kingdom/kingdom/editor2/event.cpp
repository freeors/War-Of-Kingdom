...
void tevent::tfilter_::update_to_ui_special(HWND hdlgP) const
{
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_SIDE), dgettext_2_ansi("kingdom-lib", "Side"));
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_CONTROLLER), dgettext_2_ansi("kingdom-lib", "Controller"));
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_TYPE), dgettext_2_ansi("kingdom-lib", "troop^Type"));
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_FAMILY), dgettext_2_ansi("kingdom-lib", "Family"));
...
BOOL On_DlgEventKillInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	strstr.str("");
	strstr << dgettext_2_ansi("kingdom-lib", "Hero");
...
BOOL On_DlgEventMessageInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_HERO), dgettext_2_ansi("kingdom-lib", "Hero"));
	...
	if (lpNMHdr->code == NM_CLICK) {
		if ((lpNMHdr->idFrom == IDC_LV_EVENTUNIT_TRAIT) && (lpnmitem->ptAction.x <= 14)) {
			...
			if (lpNMHdr->idFrom == IDC_LV_EVENTUNIT_TRAIT) {
				strstr << _("Traits") << "(" << editor_config::ListView_GetCheckedCount(lpNMHdr->hwndFrom) << ")";
				Static_SetText(GetDlgItem(hdlgP, IDC_STATIC_TRAITS), utf8_2_ansi(strstr.str().c_str()));
			}
		}
