...
void prepare_popup_menu()
{
	...
	AppendMenu(gdmgr._hpopup_new, MF_STRING, IDM_NEW_CAMPAIGN, dgettext_2_ansi("kingdom-lib", "Campaign"));

	// mod
	AppendMenu(gdmgr._hpopup_new, MF_SEPARATOR, 0, NULL);
	int index = 0;
	for (std::vector<tmod_copier>::const_iterator it = mod_copiers.begin(); it != mod_copiers.end(); ++ it) {
		const tmod_copier& mod = *it;

		HMENU hpopup = CreatePopupMenu();

		symbols["mod_res_path"] = mod.get_path(tmod_copier::res_tag);
		symbols["mod_patch_path"] = mod.get_path(tmod_copier::patch_tag);
	...
	for (std::vector<tapp_copier>::const_iterator it = app_copiers.begin(); it != app_copiers.end(); ++ it) {
		...
		AppendMenu(hpopup, MF_STRING, IDM_EXTRA_APP0 + index, utf8_2_ansi(app.name().c_str()));

		AppendMenu(gdmgr._hpopup_new, MF_POPUP, (UINT_PTR)(hpopup), app.app.c_str());

		gdmgr._hpopup_app[index ++] = hpopup;
	}
...
static void init_locale()
{
	...
	bindtextdomain ("wesnoth-hero", intl_dir.c_str());
	// chage default codeset to utf-8.
	bind_textdomain_codeset ("wesnoth-hero", "UTF-8");

	bindtextdomain("wesnoth-card", intl_dir.c_str());
	bind_textdomain_codeset("wesnoth-card", "UTF-8");

	bindtextdomain("editor-lib", intl_dir.c_str());
	bind_textdomain_codeset("editor-lib", "UTF-8");
	textdomain("editor-lib");
...
*/
	...
	game_config::init("editor", "Rose", "#rose", false, true);
