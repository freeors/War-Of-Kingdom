...
BOOL extract_kingdom_star_patch(tmod_copier& mod_config, const std::string& kingdom_star, const std::string& kingdom_star_patch);
...
void extract_app(tapp_copier& copier)
{
	...
	int retval = MessageBox(NULL, ss.str().c_str(), utf8_2_ansi(_("Confirm generate")), MB_YESNO | MB_DEFBUTTON2);
	if (retval == IDYES) {
		bool fok = copier.opeate_file();
...
bool check_wok_root_folder(const std::string& folder)
{
	...
	bool apps_res = is_apps_res(folder);
#ifndef _ROSE_EDITOR
	return apps_res; 
#else
	return !apps_res;
...
void On_DlgDDescCommand(HWND hdlgP, int id, HWND hwndCtrl, UINT codeNotify)
{
	...
	case IDM_EXTRA_APP8:
	case IDM_EXTRA_APP9: {
		tapp_copier& current_app = app_copiers[id - IDM_EXTRA_APP0];
	...
	case IDM_MOD_RES3:
	case IDM_MOD_RES4: {
		tmod_copier& current_mod = mod_copiers[id - IDM_MOD_RES0];
		...
		retval = MessageBox(hdlgP, text, strstr.str().c_str(), MB_YESNO | MB_DEFBUTTON2);
		if (retval == IDYES) {
			fok = generate_kingdom_mod_res(current_mod, gdmgr._menu_text, current_mod.get_path(tmod_copier::patch_tag), current_mod.get_path(tmod_copier::res_tag));
	...
	case IDM_MOD_PATCH3:
	case IDM_MOD_PATCH4: {
		tmod_copier& current_mod = mod_copiers[id - IDM_MOD_PATCH0];
		symbols["mod"] = current_mod.name();
		symbols["mod_patch_path"] = current_mod.get_path(tmod_copier::patch_tag);
		strcpy(text, utf8_2_ansi(vgettext2("Do you want to extract $mod different files to $mod_patch_path?", symbols).c_str()));
		strstr.str("");
		strstr << utf8_2_ansi(_("Confirm generate"));
		retval = MessageBox(hdlgP, text, strstr.str().c_str(), MB_YESNO | MB_DEFBUTTON2);
		if (retval == IDYES) {
			fok = extract_kingdom_star_patch(current_mod, gdmgr._menu_text, current_mod.get_path(tmod_copier::patch_tag));
			symbols["src"] = gdmgr._menu_text;
			symbols["dst"] = current_mod.get_path(tmod_copier::patch_tag);
			symbols["result"] = fok? "Success": "Fail";
...
BOOL On_DlgDDescNotify(HWND hdlgP, int DlgItem, LPNMHDR lpNMHdr)
{
	...
	if (lpNMHdr->code == NM_RCLICK) {
		...
		// star
		if (!can_execute_tack(TASK_NEW) || strcasecmp(gdmgr._menu_text, game_config::path.c_str())) {
			for (int i = 0; i < (int)mod_copiers.size(); ++ i) {
				HMENU hpopup = gdmgr._hpopup_mod[i];
				EnableMenuItem(gdmgr._hpopup_new, (UINT_PTR)(hpopup), MF_BYCOMMAND | MF_GRAYED);
			}
			for (int i = 0; i < (int)app_copiers.size(); ++ i) {
				HMENU hpopup = gdmgr._hpopup_app[i];
				EnableMenuItem(gdmgr._hpopup_new, (UINT_PTR)(hpopup), MF_BYCOMMAND | MF_GRAYED);
			}
		} else {
			for (int i = 0; i < (int)mod_copiers.size(); ++ i) {
				tmod_copier& current_mod = mod_copiers[i];
				HMENU hpopup = gdmgr._hpopup_mod[i];
			...
			for (int i = 0; i < (int)app_copiers.size(); ++ i) {
				HMENU hpopup = gdmgr._hpopup_app[i];
				EnableMenuItem(gdmgr._hpopup_new, (UINT_PTR)(hpopup), MF_BYCOMMAND | MF_ENABLED);
				EnableMenuItem(hpopup, IDM_EXTRA_APP0 + i, MF_BYCOMMAND | MF_ENABLED);
			}
		}
