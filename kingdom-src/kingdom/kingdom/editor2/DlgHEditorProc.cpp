...
BOOL On_DlgWGenInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	strstr.str("");
	strstr << "Cost";
	strcpy(text, utf8_2_ansi(dgettext("kingdom-lib", strstr.str().c_str())));
	lvc.pszText = text;
	ListView_InsertColumn(hctl, column ++, &lvc);
...
BOOL On_DlgHeroEditInitDialog(HWND hdlgP, HWND hwndFocus, LPARAM lParam)
{
	...
	set_language_text(hdlgP, IDC_STATIC_SKILL, "kingdom-lib", "Skill");
	...
	set_language_text(hdlgP, IDC_STATIC_RELATION, "kingdom-lib", "Relation");
