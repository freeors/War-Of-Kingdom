...
tactic_anim_lock::~tactic_anim_lock()
{
	...
	if (!scroll_) {
		...
		if (!require_anim) {
			...
			for (std::vector<unit_animator::anim_elem>::const_iterator it = animated_units.begin(); it != animated_units.end(); ++ it) {
				...
				const unit& my_unit = *dynamic_cast<const unit*>(it->my_unit);
	...
	if (require_anim && tactic_animator.get_end_time() != INT_MIN) {
		...
		tactic_animator.wait_for_end(*resources::controller);
	...
	if (tactic_animator.get_end_time() != INT_MIN) {
		...
		for (std::vector<unit_animator::anim_elem>::iterator it = animated_units.begin(); it != animated_units.end(); ++ it, index ++) {
			...
			if (std::find(defender_.begin(), defender_.end(), anim.my_unit) != defender_.end()) {
				std::map<const unit*, unit_animator::anim_elem*>::const_iterator find = defender_pos.find(dynamic_cast<unit*>(anim.my_unit));
				if (find != defender_pos.end()) {
					...
					defender_pos.insert(std::make_pair(dynamic_cast<const unit*>(anim.my_unit), &anim));
				}
			} else {
				const unit_animation* anim2 = dynamic_cast<const unit_animation*>(anim.anim);
				if (anim2->event0() == "attack") {
					attacker.push_back(dynamic_cast<unit*>(anim.my_unit));
				}
			}
		}
		...
		for (std::vector<unit_animator::anim_elem>::iterator it = animated_units.begin(); it != animated_units.end(); index ++) {
			unit_animator::anim_elem& anim = *it;
			std::string e = dynamic_cast<const unit_animation*>(anim.anim)->event0();
			if (e != "attack" && e != "defend") {
				if (std::find(attacker.begin(), attacker.end(), anim.my_unit) != attacker.end()) {
					desire_erase[index] = true;
				} else if (defender_pos.find(dynamic_cast<const unit*>(anim.my_unit)) != defender_pos.end()) {
					desire_erase[index] = true;
				}
			}
...
{
	if (const animation* tpl = anim2::anim(anim2::HSCROLL_TEXT)) {
		...
		anim.replace_static_text("name", "__text", vgettext2("Capture $city", symbols));
	...
	const std::string& att_text, artifical* effecting_tactic, const tformation& defender_formation)
{
	...
	if (force_scroll || point_in_rect_of_hexes(attacker.get_location().x, attacker.get_location().y, draw_area) || has_eyeshot_in_defs) {
		...
		for (size_t i = 0; i < def_size; i ++) {
			...
			animator->add_animation2(def_ptr_vec[i], defender_anim, def_ptr_vec[i]->get_location(),
		...
		if (defender_formation.valid(false)) {
			...
			const unit_animation* heal_anim = dynamic_cast<const unit_animation*>(anim2::anim(anim2::FORMATION_DEFEND));
			if (heal_anim) {
				animator->add_animation2(defender_formation.u_, heal_anim, defender_formation.u_->get_location(), 
		...
		if (!cast_tactic_state) {
			...
			normal_animator.wait_until(*resources::controller, 0);
			...
			while (damage_left[0] > 0 && !normal_animator.would_end()) {
				...
				normal_animator.wait_until(*resources::controller, normal_animator.get_animation_time_potential() + 50);
			}

			normal_animator.wait_for_end(*resources::controller);
	 ...
	 std::vector<std::string>& hit_text_vec)
{
	...
	if (force_scroll || attacker_in_eyeshot || has_eyeshot_in_defs) {
		...
		if (attacker) {
			const unit_animation* attacker_anim = dynamic_cast<const unit_animation*>(anim2::anim(anim2::MAGIC));
			if (attacker_anim) {
				animator.add_animation2(attacker, attacker_anim);
				animated_units.insert(attacker);
			}
		}
		...
		if (type == "blade" || type == "pierce" || type == "impact") {
			defender_anim = dynamic_cast<const unit_animation*>(anim2::anim(anim2::BLADE));
		} else {
			defender_anim = dynamic_cast<const unit_animation*>(anim2::anim(anim2::FIRE));
		}
		for (size_t i = 0; i < def_size; i ++) {
			...
			animator.add_animation2(defender, defender_anim, defender->get_location(),
				true, false, text_vec[i], display::rgb(255, 0, 0));
			defender_anim_vec.push_back(defender_anim);
		...
		animator.wait_until(*resources::controller, 0);
		std::vector<int> damage_left = damage_vec;
		// why use this while? --make HP bare damage more smooth.
		while (damage_left[0] > 0 && !animator.would_end()) {
			...
			animator.wait_until(*resources::controller, animator.get_animation_time_potential() + 50);
		}
...
void unit_heal2(std::vector<team>& teams, unit_map& units, unit* doctor, unit& patient, int healing)
{
	...
	if (force_scroll || toucher_in_eyeshot) {
		...
		const unit_animation* heal_anim = dynamic_cast<const unit_animation*>(anim2::anim(anim2::HEAL));
		if (heal_anim) {
			animator.add_animation2(&patient, heal_anim, patient_loc, false, false, lexical_cast<std::string>(healing), display::rgb(0, 255, 0));
...
void unit_destruct(std::vector<team>& teams, unit_map& units, unit* attacker, artifical& defender, const std::string& text, artifical* effecting_tactic)
{
	...
	if (force_scroll || toucher_in_eyeshot) {
		...
		const unit_animation* destruct_anim = dynamic_cast<const unit_animation*>(anim2::anim(anim2::DESTRUCT));
		if (destruct_anim) {
			animator.add_animation2(&defender, destruct_anim, defender.get_location(), false, false, text, display::rgb(255, 0, 0));
			animated_units.insert(&defender);
		}
...
{
	game_display* disp = resources::screen;
	if (const animation* tpl = anim2::anim(anim2::CARD)) {
		int id = disp->insert_area_anim(*tpl);
		animation& anim = disp->area_anim(id);

		anim.replace_image_name("id", "__id.png", c.image());
		new_animation_frame();
		anim.start_animation(0);
...
void tactic_start(hero& h)
{
	...
	game_display* disp = resources::screen;
	if (const animation* tpl = anim2::anim(anim2::TACTIC)) {
		int id = disp->insert_area_anim(*tpl);
		animation& anim = disp->area_anim(id);

		anim.replace_image_name(null_str, "__bg.png", t.bg_image());
		anim.replace_image_name("id", "__id.png", h.image(true));
		anim.replace_static_text("name", "__hero", h.name());
		anim.replace_static_text("tactic", "__tactic", t.name());
		anim.replace_static_text("description", "__description", t.description());
...
void formation_attack_start(const tformation& formation)
{
	...
	game_display* disp = resources::screen;
	if (const animation* tpl = anim2::anim(anim2::FORMATION_ATTACK)) {
		int id = disp->insert_area_anim(*tpl);
		animation& anim = disp->area_anim(id);
		std::stringstream strstr;
		...
		anim.replace_image_name(null_str, "__bg.png", profile.bg_image());
		anim.replace_image_name("id", "__id.png", formation.u_->master().image(true));
		anim.replace_static_text("formation", "__formation_name", profile.name());
		anim.replace_static_text("description", "__formation_description", profile.description());
		new_animation_frame();
		anim.start_animation(0);
...
void global_anim_2(int type, const std::string& id1, const std::string& id2, const std::string& text, const SDL_Color& color)
{
	...
	game_display* disp = resources::screen;
	if (const animation* tpl = anim2::anim(type)) {
		int id = disp->insert_area_anim(*tpl);
		animation& anim = disp->area_anim(id);

		anim.replace_image_name(null_str, "__id1.png", id1);
		anim.replace_image_name("id", "__id2.png", id2);
		anim.replace_static_text("text", "__text", text);
		if (color != font::GOOD_COLOR) {
			anim.replace_int("text", "text_color", display::rgb(0, 255, 0), display::rgb(color.r, color.g, color.b));
		}
		new_animation_frame();
		anim.start_animation(0);
...
void perfect_anim()
{
	...
	game_display* disp = resources::screen;
	if (const animation* tpl = anim2::anim(anim2::PERFECT)) {
		int id = disp->insert_area_anim(*tpl);
		animation& anim = disp->area_anim(id);
		std::stringstream strstr;

		anim.replace_static_text(null_str, "__perfect", _("perfect^Perfect"));
...
int start_title_screen_anim()
{
	display* disp = display::get_singleton();
	int id = -1;
	if (const animation* tpl = anim2::anim(anim2::TITLE_SCREEN)) {
		id = disp->insert_area_anim(*tpl);
		animation& anim = disp->area_anim(id);
		std::stringstream strstr;

		// anim.replace_static_text("__perfect", _("perfect^Perfect"));
...
void stratagem_anim(const ttechnology& stratagem, const std::string& image, bool up)
{
	...
	game_display* disp = resources::screen;
	if (const animation* tpl = anim2::anim(up? anim2::STRATAGEM_UP: anim2::STRATAGEM_DOWN)) {
		int id = disp->insert_area_anim(*tpl);
		animation& anim = disp->area_anim(id);
		std::stringstream strstr;

		anim.replace_static_text("name", "__name", stratagem.name());
		anim.replace_image_name(null_str, "__image", image);
...
void unit_income(unit& u, int income)
{
	...
	const unit_animation* income_anim_tpl = dynamic_cast<const unit_animation*>(anim2::anim(anim2::INCOME));
	if (!income_anim_tpl) {
		return;
	}
	...
	strstr.str("");
	strstr << "0~" << xoffset;
	income_anim.replace_progressive("missile", "x", "5555", strstr.str());
	income_anim.replace_progressive("missile", "x", "6666", str_cast(xoffset));
	strstr.str("");
	strstr << "-60~" << yoffset;
	income_anim.replace_progressive("missile", "y", "5555", strstr.str());
	income_anim.replace_progressive("missile", "y", "6666", str_cast(yoffset));
	...
	if (force_scroll || point_in_rect_of_hexes(loc.x, loc.y, draw_area)) {
		...
		animator.add_animation2(&u, &income_anim, loc, !text.empty(), false, text, color);
...
{
	const animation* tpl = anim2::anim(anim2::LOCATION);
	if (!tpl || locs.empty()) {
		return;
	}
	...
	for (std::vector<tlocation_anim>::const_iterator it = locs.begin(); it != locs.end(); ++ it) {
		...
		anim.replace_progressive(null_str, "offset_x", "5555", str_cast(1.0 * it->xoffset / 100));
		anim.replace_progressive(null_str, "offset_y", "5555", str_cast(1.0 * it->yoffset / 100));
