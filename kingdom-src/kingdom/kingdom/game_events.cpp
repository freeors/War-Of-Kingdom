...
namespace game_events {
	...
	void show_relation_message(unit_map& units, hero_map& heros, hero& h1, hero& h2, int carry_to)
	{
		...
		if (carry_to == hero::FEELING_CONSORT) {
			if (!h2.is_consort(h1)) {
				message = vgettext2("I'm happy! I have married with $second.", symbols);
				incident = game_events::INCIDENT_MARRY;
			}
		} else if (carry_to == hero::FEELING_OATH) {
			if (!h2.is_oath(h1)) {
				message = vgettext2("I'm happy! I have oathed with $second.", symbols);
	...
	bool confirm_carry_to(hero& h1, hero& h2, int carry_to)
	{
		...
		symbols["second"] = tintegrate::generate_format(h2.name(), tintegrate::hero_color);
		if (carry_to == hero::FEELING_CONSORT) {
			str = vgettext2("$first want to marry with $second, do you agree?", symbols);
		} else if (carry_to == hero::FEELING_OATH) {
			str = vgettext2("$first want to oath with $second, do you agree?", symbols);
		}
