/* $Id: attack_prediction_display.cpp 40489 2010-01-01 13:16:49Z mordante $ */
/*
	...
	dialog_height_(0)
{
	...
	get_hp_prob_vector(attacker_combatant.hp_, hp_prob_vector);
	...
	get_hp_prob_vector(defender_combatant.hp_, hp_prob_vector);
									      ...
									      int& left_strings_width, int& right_strings_width, int& strings_width)
{
	...
	if(stats.weapon != NULL) {
		...
		weapon->set_specials_context(&u, &opp, *resources::units, stats.is_attacker, opp_weapon);
...
void battle_prediction_pane::get_hp_prob_vector(const double hp, std::vector<std::pair<int, double> >& hp_prob_vector)
{
...
*/
	if (hp > 0) {
		prob_hp_vector.push_back(std::pair<double, int>(1, (int)hp));
	}
