...
#include "gui/auxiliary/widget_definition/hexmap.hpp"
#include "gui/auxiliary/window_builder/hexmap.hpp"
...
const surface thexmap::get_image(const int w, const int h)
{
	if (!terrain_) {
		return NULL;
	}

	if (callback_size_change_ && (w != keep_size_.x || h != keep_size_.y)) {
		callback_size_change_(this);
		keep_size_.x = w;
		keep_size_.y = h;
	}
	...
	try {
		...
		if (!surf.null() && (surf.get()->w != w || surf.get()->h != h)) {
...
void thexmap::impl_draw_background(surface& frame_buffer)
{
	if (!terrain_) return;
	assert(terrain_);
...
void thexmap::impl_draw_background(
...
const std::string& thexmap::get_control_type() const
{
	static const std::string type = "hexmap";
	return type;
}
