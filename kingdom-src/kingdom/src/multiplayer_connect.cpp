/* $Id: multiplayer_connect.cpp 47582 2010-11-16 23:28:08Z ai0867 $ */
/*
...
connect::side::side(connect& parent, const config& cfg, int index) :
	...
	combo_feature_(parent.disp(), parent.side_features_),
	...
	enabled_(!parent_->params_.saved_game && cfg["leader"].empty()), changed_(false),
	llm_(index, parent.heros_, *parent.era_cfg_, parent.factions_, enabled_ ? &combo_faction_ : NULL, enabled_ ? &combo_gender_ : NULL)
{
...
*/
	...
	if (!enabled_) {
		...
		pseudo_factions.push_back(leader.name());
		...
		if (leader.side_feature_ != HEROS_NO_SIDE_FEATURE) {
			...
			combo_feature_.set_selected(COMBO_FEATURES_MIN_VALID + itor - features.begin());
		} else {
			combo_feature_.set_selected(COMBO_FEATURES_NONE);
		}
...
connect::side::side(const side& a) :
	...
	combo_faction_(a.combo_faction_), combo_feature_(a.combo_feature_), combo_gender_(a.combo_gender_),
...
{
	llm_.set_color(color_);
	llm_.set_leader_combo((enabled_ && leader_.empty()) ? &combo_faction_ : NULL);
...
void connect::side::add_widgets_to_scrollpane(gui::scrollpane& pane, int pos)
{
	...
	pane.add_widget(&orig_controller_,  20 + (combo_controller_->width() - orig_controller_.width()) / 2,
									    35 + pos + (combo_feature_.height() - orig_controller_.height()) / 2);
	pane.add_widget(&combo_ai_algorithm_, 20, 35 + pos);
	pane.add_widget(&combo_faction_, 135, 5 + pos);
	pane.add_widget(&combo_feature_,  135, 35 + pos);
...
void connect::side::process_event()
{
	...
	if (combo_feature_.changed() && combo_feature_.selected() >= 0) {
		// parent_->update_force_list();
		changed_ = true;
	}
...
config connect::side::get_config() const
{
	...
	if (enabled_) {
		if (llm_.get_leader() != RANDOM_FORCE) {
			// add content in [faction][/faction] to this side
			const config& faction_cfg = *parent_->factions_[llm_.get_leader()].first;
	...
	*/
	...
	res["selected_feature"] = combo_feature_.selected();
...
void connect::side::resolve_random()
{
	...
	if (llm_.get_leader() == RANDOM_FORCE) {
		...
		if (!types.empty()) {
			do {
				lchoice = rand() % combo_faction_.count();
				combo_faction_.set_selected(lchoice? lchoice: 1);
			} while (parent_->factions_[llm_.get_leader()].second >= 0);
			parent_->factions_[llm_.get_leader()].second = index_;
	...
	if (combo_feature_.selected() == COMBO_FEATURES_RANDOM) {
		do {
			lchoice = rand() % combo_feature_.count();
		} while (lchoice < COMBO_FEATURES_MIN_VALID);
		combo_feature_.set_selected(lchoice);
	}
	const config& faction_cfg = *parent_->factions_[llm_.get_leader()].first;
	hero& leader = parent_->heros_[faction_cfg["leader"].to_int()];
	int selected_feature = combo_feature_.selected();
	if (selected_feature == COMBO_FEATURES_NONE) {
...
connect::connect(hero_map& heros, game_display& disp, gamemap& gmap, const config& game_config,
...
void connect::load_game()
{
	...
	const config& topest_cfg = game_config();
	factions_.clear();
...
void connect::update_force_list()
{
	// clear flag that my side used
	for (std::vector<std::pair<const config *, int> >::iterator v = factions_.begin(); v != factions_.end(); ++ v) {
		v->second = RANDOM_FORCE;
	}
	int index = 0;
	for (side_list::iterator itor = sides_.begin(); itor != sides_.end(); ++itor, index ++) {
		if (itor->llm_.get_leader() != RANDOM_FORCE) {
			factions_[itor->llm_.get_leader()].second = index;
		}
	}
	for (side_list::iterator itor = sides_.begin(); itor != sides_.end(); ++itor) {
		itor->llm_.update_leader_list(itor->combo_faction_.selected());
	}
}
