...
bool enemy_zoc(std::vector<team> const &teams, map_location const &loc,
               team const &viewing_team, int side, bool see_all=false, bool ignore_city = false);
...
struct cost_calculator
{
	...
	inline double getUnitHoldValue() const { return (424242.0); }
};
...
void reallocate_pq(size_t width, size_t height);
void release_pq();
}
