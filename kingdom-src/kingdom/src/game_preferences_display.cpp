...
//
	...
	gui::button fullscreen_button_, scroll_to_action_button_,turbo_button_, eng_file_name_button_,
...
preferences_dialog::preferences_dialog(display& disp, const config& game_cfg)
	: gui::preview_pane(disp.video()),
	  ...
	  eng_file_name_button_(disp.video(), _("Localize save file name"), gui::button::TYPE_CHECK),
	  ...
	  tab_(GENERAL_TAB), disp_(disp), game_cfg_(game_cfg), parent(NULL)
{
	...
	eng_file_name_button_.set_check(!eng_file_name());
	eng_file_name_button_.set_help_string(_("Use local language to name save file"));

	interrupt_when_ally_sighted_button_.set_check(interrupt_when_ally_sighted());
	interrupt_when_ally_sighted_button_.set_help_string(_("Sighting an allied unit interrupts your unit't movement"));
...
handler_vector preferences_dialog::handler_members()
{
	...
	h.push_back(&eng_file_name_button_);
...
void preferences_dialog::update_location(SDL_Rect const &rect)
{
	...
	const int short_interline = 22; // 21
	...
	ypos += item_interline; eng_file_name_button_.set_location(rect.x, ypos); // eng_file_name_button_.enable(false);
...
void preferences_dialog::process_event()
{
	if (tab_ == GENERAL_TAB) {
		...
		if (eng_file_name_button_.pressed())
			set_eng_file_name(!eng_file_name_button_.checked());
...
void preferences_dialog::set_selection(int index)
{
	...
	turbo_slider_label_.enable(turbo());
	turbo_slider_.enable(turbo());
	eng_file_name_button_.hide(hide_general);
