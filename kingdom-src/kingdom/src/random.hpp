...
{
public:
	enum TYPE {UNGZIP = 1, START, INIT_SIDE, DO_COMMONER, END_TURN, INIT_AI, RECRUIT, EXPEDITE, DISBAND, MOVE_HEROS, BELONG_TO, 
		BUILD, CAST_TACTIC, COUNTDOWN_UPDATE, MOVEMENT, ATTACK, RANDOM_NUMBER, CHOOSE, LABLE, RENAME, SPEAK, 
		EVENT, UNIT_CHECKSUM, CHECKSUM_CHECK, INCHING_BLOCK, CLEAR_LABELS, DIPLOMATISM, 
		FINAL_BATTLE, INPUT, HERO_FIELD, ADD_CARD, ERASE_CARD, ARMORY, INTERIOR, ASSEMBLE_TREASURE, 
		RPG_EXCHANGE, ING_TECHNOLOGY};
	struct command {
		TYPE type;
		unsigned int flags;
	};
