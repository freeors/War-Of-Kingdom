/* $Id: variable.cpp 52533 2012-01-07 02:35:17Z shadowmaster $ */
/*
...
void scoped_player::activate()
{
	config tmp_cfg;
	tmp_cfg["hero"] = lexical_cast<std::string>(hero_);
	tmp_cfg["side"] = lexical_cast<std::string>(side_ + 1);
	tmp_cfg["shroud"] = shroud_? "yes": "no";
	tmp_cfg["fog"] = fog_? "yes": "no";
	store(tmp_cfg);
}

void scoped_player::set_shroud(bool shroud, bool fog)
{
	shroud_ = shroud;
	fog_ = fog;
}

void scoped_hero::activate()
{
	const hero& h = hmap_[number_];

	config &tmp_cfg = store();
	tmp_cfg["number"] = h.number_;
}
