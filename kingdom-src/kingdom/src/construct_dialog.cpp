/* $Id: construct_dialog.cpp 47837 2010-12-05 18:09:51Z mordante $ */
/*
...
const int dialog::message_font_size = font::SIZE_NORMAL;
...
void dialog::set_menu(menu *const m) 
{ 
	if (menu_ && menu_ != empty_menu) {
		// menu析构函数不会擦除菜单，需用在析构之前显示调用一次擦除背景（即恢复旧有图像）
		// menu_->bg_restore();
		menu_->hide();
		delete menu_;
...
dialog::dimension_measurements dialog::layout(int xloc, int yloc)
{
	...
	const int menu_hpadding = font::relative_size((dim.message.h > 0 && use_menu) ? 5 : 0);
...
int dialog::process(dialog_process_info &info)
{
	...
	if ((menu_->selection() != info.selection) || menu_->checks_dirty() || info.first_time) {
