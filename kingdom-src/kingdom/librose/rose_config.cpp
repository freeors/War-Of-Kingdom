...
namespace game_config
{
	...
	const std::string rose_version = "0.0.12";
...
void init(const std::string& _app, const std::string& msgid, const std::string& channel, bool hdpi, bool landscape)
{
	VALIDATE(!_app.empty() && _app.size() <= MAXLEN_APP && !msgid.empty(), null_str);
	app = _app;
	app_msgid = msgid;
	app_channel = channel;
	...
	VALIDATE(gui2::twidget::hdpi_ratio > 0 && gui2::twidget::hdpi_ratio < 5, null_str);
