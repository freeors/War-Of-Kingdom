...
#include "gui/auxiliary/widget_definition/theme.hpp"
...
const surface ttheme::get_image(const int w, const int h)
{
	if (!terrain_) {
		return NULL;
	}
...
void ttheme::impl_draw_background(surface& frame_buffer)
{
	if (!terrain_) return;
	assert(terrain_);
...
void ttheme::impl_draw_background(
...
const std::string& ttheme::get_control_type() const
{
