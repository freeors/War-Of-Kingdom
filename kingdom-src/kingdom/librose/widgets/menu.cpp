/* $Id: menu.cpp 40489 2010-01-01 13:16:49Z mordante $ */
/*
...
menu::~menu()
{
	for (std::map<size_t, button*>::iterator itor = checks_.begin(); itor != checks_.end(); ++ itor) {
		delete itor->second;
	}
}

handler_vector menu::handler_members()
{
	handler_vector h = scrollarea::handler_members();
	for (std::map<size_t, button*>::iterator itor = checks_.begin(); itor != checks_.end(); ++ itor) {
		h.push_back(itor->second);
	}
	return h;
}
...
{
	for (std::vector<std::string>::const_iterator itor = items.begin(); itor != items.end(); ++itor) {
		...
		if (first_item.empty() == false && first_item.find(CHECK_PREFIX) == 0) {
			checks_.insert(std::pair<size_t, button*>(id, new button(video(), "", button::TYPE_CHECK, "", button::DEFAULT_SPACE, true, this, NULL, id)));
			if (first_item.size() >= CHECK_PREFIX.size() + 1 && first_item.at(CHECK_PREFIX.size()) == 'Y') {
				checks_[id]->set_check(true);
			}
		}
	}
	...
	column_pixels_ = std::accumulate(widths.begin(), widths.end(), 0);
...
void menu::update_scrollbar_grip_height()
{
	...
	set_hfull_size(column_pixels_);
	set_hshown_size(max_hpixels_onscreen() - (items_.size() > max_items_onscreen()? scrollbar_width(): 0));
}

void menu::update_size()
{
	unsigned int h = heading_height();
	for (size_t i = get_position(), i_end = std::min(items_.size(), i + max_items_onscreen()); i < i_end; ++i) {
		h += get_item_rect(i).h;
	}
	if (column_pixels_ > max_hpixels_onscreen()) {
		h += hscrollbar_height();
	} else {
		// if height() isn't integer times line-height, below std::max will result in gap at end. 
		h = std::max(h, height());
	}
...
#ifdef USE_TINY_GUI
	size_t max_height = (max_height_ == -1 ? (video().gety()*55)/100 : max_height_) - heading_height();
#else
	size_t max_height = (max_height_ == -1 ? (video().gety()*90)/100 : max_height_) - heading_height();
#endif

	if (column_pixels_ > max_hpixels_onscreen()) {
		max_height -= hscrollbar_height();
	}
...
size_t menu::max_hpixels_onscreen() const
{
	return max_width_ == -1? (video().getx() * 2) / 3: max_width_;
}
...
SDL_Rect menu::style::item_size(const std::string& item) const {
	...
	for (std::vector<std::string>::const_iterator it = img_text_items.begin();
		...
		} else if (!str.empty() && str.find(CHECK_PREFIX) == 0) {
			const std::string image_name("buttons/checkbox.png");
			surface const img = get_item_image(image_name);
			if (img != NULL) {
				res.w += img->w;
				res.h = std::max<int>(img->h, res.h);
...
{
	// check box
	if (!item.empty() && (item.find(CHECK_PREFIX) == 0)) {
		return true;
	}
	// general image
...
void menu::draw_row(const size_t row_index, const SDL_Rect& rect, ROW_TYPE type)
{
	...
	SDL_Rect column = loc;
	size_t hiden_width = 0, front_should_hide_width = get_hposition();
	bool always;
	...
	for (size_t i = 0; i != row.size(); ++i) {
		...
		always = ((int)i < always_columns_)? true: false;

		int xpos_adjusted = xpos - std::min(hiden_width, front_should_hide_width);
		int width_adjusted = widths[i];
		if (!always && front_should_hide_width > hiden_width) {
			width_adjusted -= front_should_hide_width - hiden_width;
		}
		if (xpos_adjusted + width_adjusted > loc.x + loc.w) {
			width_adjusted = loc.x + loc.w - xpos_adjusted;
		}

		if (type == HEADING_ROW && highlight_heading_ == int(i)) {
			draw_solid_tinted_rectangle(xpos_adjusted, rect.y, width_adjusted, rect.h, 255, 255, 255, 0.3, video().getSurface());
		}
		...
		for (std::vector<std::string>::const_iterator it = img_text_items.begin(); it != img_text_items.end(); ++it) {
			str = *it;
			if (!str.empty() && (str.find(CHECK_PREFIX) == 0)) {
				// check box
				checks_[items_[row_index].id]->set_location(xpos, rect.y + (rect.h - checks_[items_[row_index].id]->height()) / 2);
				checks_[items_[row_index].id]->hide(false);
				// 强制调用一次set_dirty以把复选设置到DIRTY
				// ...菜单第一次显示, 没有以下这个set_dirty, 是正确的;
				// 菜单之后显示, 像选择另一项, 由于set_location设置的是同一个矩形,不会把复选框设置到DIRTY状态,
				// 结果是复选框的draw什么事都不干, 导致复选框消失
				checks_[items_[row_index].id]->set_dirty(true);
				const size_t w = checks_[items_[row_index].id]->width();
				xpos += w;
				xpos_adjusted += w;
				width_adjusted -= w;

			} else if (!str.empty() && str[0] == IMAGE_PREFIX) {
				const std::string image_name(str.begin()+1,str.end());
				const surface img = style_->get_item_image(image_name);
				const int remaining_width = max_width_ < 0 ? area.w: std::min<int>(max_width_, ((lang_rtl)? xpos - rect.x : rect.x + rect.w - xpos));
				...
				column.x = xpos_adjusted;
				const bool has_wrap = (str.find_first_of("\r\n") != std::string::npos);
				// prevent hide calculation if there is any line wrapping
				const std::string to_show = (use_ellipsis_ && !has_wrap)?
					font::make_text_hide(str, always, front_should_hide_width, hiden_width, style_->get_font_size(), loc.w - (column.x - rect.x) - 2*style_->get_thickness(), false, true): str;
				
				const SDL_Rect& text_size = font::text_area(str, style_->get_font_size());
				const size_t y = rect.y + (rect.h - text_size.h)/2;
				font::draw_text(&video(), column, style_->get_font_size(), font::NORMAL_COLOR, to_show, column.x, y);
				
				if (type == HEADING_ROW && sortby_ == int(i)) {
					const surface sort_img = image::get_image(sortreversed_? "misc/sort-arrow.png": "misc/sort-arrow-reverse.png");
		...
		if (grid_) {
			SDL_Rect area;
			SDL_Rect clip = {xpos_adjusted, rect.y, width_adjusted, rect.h};

			surface image = image::get_image("misc/selection3-n-border-right.png");
			area.x = xpos_adjusted + width_adjusted - image->w;
			area.y = rect.y;
			do {
				video().blit_surface(area.x, area.y, image, NULL, &clip);
				area.y += image->h;
		...
		if (use_ellipsis_ && !always && hiden_width < front_should_hide_width) {
			hiden_width += widths[i];
		}
	}
}
...
void menu::draw_contents()
{
	...
	for(size_t i = 0; i != item_pos_.size(); ++i) {
		SDL_Rect item_rect = get_item_rect(i);
		if (item_rect.h && item_rect.w) { 
			style_->draw_row(*this, item_pos_[i], item_rect, (!out_ && item_pos_[i] == selected_) ? SELECTED_ROW : NORMAL_ROW);
		} else if (checks_.size() > i) {
			// 这里定位具体复选框直接用i,而不是items[i].id.
			// 参考以上的draw_row参数, item_pos_[i]得出的正是row_index值
			checks_[i]->hide(true);
		}
	}
}
...
int menu::hit_column(int x) const
{
	...
	std::vector<int> const &widths = column_widths();
	if (!widths.size() || (x < loc.x) || (x > loc.x + loc.w)) {
		return -1;
	}
	x -= loc.x + widths[0];
	int adjusted_x = use_ellipsis_? x + get_hposition(): x;

	int col, col_end = widths.size();
	for (col = 0; col < col_end; col ++) {
		if (col < always_columns_) {
			if (x < 0) {
				return col;
			}
		} else {
...
SDL_Rect menu::get_item_rect_internal(size_t item) const
{
	...
	if (item < first_item_on_screen || size_t(item) >= first_item_on_screen + max_items_onscreen()) {
		return empty_rect;
	}
	...
	SDL_Rect res = { loc.x, y, loc.w, int(get_item_height(item)) };
...
std::vector<size_t> menu::checked() const
{
	std::vector<size_t> checked_id;

	for (std::map<size_t, button*>::const_iterator itor = checks_.begin(); itor != checks_.end(); ++ itor) {
		if (itor->second->checked()) {
			checked_id.push_back(itor->first);
		}
	}
	return checked_id;
}

bool menu::before_press(const void* menu, const size_t btnidx)
{
	if (max_allow_checks_ == 0) {
		return true;
	}
	if (checks_[btnidx]->checked()) {
		return true;
	}
	size_t checks = 0;
	for (std::map<size_t, button*>::const_iterator itor = checks_.begin(); itor != checks_.end(); ++ itor) {
		if (itor->second->checked()) {
			if (++ checks >= max_allow_checks_) {
				return false;
			}
		}
	}
...
void menu::pressed(const void* menu, const size_t btnidx)
{
	checks_dirty_ = true;
}

void menu::set_max_allow_checks(size_t max_allow_checks)
{
	max_allow_checks_ = max_allow_checks;
}

void menu::set_always_columns(int always_columns)
{
	always_columns_ = always_columns;
}

int menu::always_columns() const
{
	return always_columns_;
}

void menu::set_grid(bool grid)
{
	grid_ = grid;
}

bool menu::grid() const
{
	return grid_;
}
