/* $Id: menu_style.cpp 40489 2010-01-01 13:16:49Z mordante $ */
/*
...
menu::imgsel_style menu::bluebg_style("misc/selection2", true,
										   ...
										   0.35, 0.0, 0.3);
*/
menu::imgsel_style menu::bluebg_style("misc/selection3", true,
										   0x000000, 0x000000, 0x333333,
										   0.35, 0.0, 0.3);
...
menu *empty_menu = NULL;
	...
	//constructors
menu::style::style() : font_size_(16 /*font::SIZE_NORMAL*/),
		cell_padding_(font_size_ * 2/5), thickness_(0),
...
size_t menu::style::get_font_size() const 
{
	return font_size_;
}

size_t menu::style::get_cell_padding() const 
{ 
...
bool menu::imgsel_style::load_images()
{
	if(!initialized_)
	{
		...
		{
			thickness_ = std::min(img_map_["border-top"]->h, img_map_["border-left"]->w);

			if (has_background_ && !load_image("background")) {
				load_failed_ = true;
			} else {
				// load_image("n-border-botleft");
				// load_image("n-border-botright");
				// load_image("n-border-topleft");
				// load_image("n-border-topright");
				load_image("n-border-left");
				load_image("n-border-right");
				load_image("n-border-top");
				load_image("n-border-bottom");
...
void menu::imgsel_style::draw_row(menu& menu_ref, const size_t row_index, const SDL_Rect& rect, ROW_TYPE type)
{
	if(!load_failed_) {
		...
		} else if (type == NORMAL_ROW && menu_ref.grid()) {
			// draw border
			surface image;
			SDL_Rect area;
			SDL_Rect clip = rect;
...
/*
			image = img_map_["n-border-bottom"];
			area.x = rect.x;
			area.y = rect.y + rect.h - thickness_;
			do {
				menu_ref.video().blit_surface(area.x,area.y,image,NULL,&clip);
