/* $Id: menu.hpp 40489 2010-01-01 13:16:49Z mordante $ */
/*
...
class menu : public scrollarea, public button_hook
{
public:
	...
	std::vector<size_t> checked() const;
	void set_max_allow_checks(size_t max_allow_checks);

	bool before_press(const void* menu, const size_t btnidx);
	void pressed(const void* menu, const size_t btnidx); 

	bool checks_dirty() const { return checks_dirty_; }
	void clear_checks_dirty() { checks_dirty_ = false; }

	void set_always_columns(int always_columns);
	int always_columns() const;
...
private:
	...
	size_t max_hpixels_onscreen() const;
	...
	std::map<size_t, button*> checks_;
	size_t max_allow_checks_;
	bool checks_dirty_;
	int always_columns_;
	bool grid_;
};
