/* $Id: theme.hpp 47265 2010-10-28 21:06:27Z mordante $ */
/*
...
public:
	...
	public:
		...
		bool button_style() const { return button_style_; }

		// below three member is valid only context menu
		void set_parent(menu* parent) { parent_ = parent; }
		const menu* parent() const { return parent_; }

		void set_pressed_btnidx(size_t btnidx) { btnidx_ = btnidx; }
		const size_t pressed_btnidx() const { return btnidx_; }
	private:
		bool context_;
		std::string title_, tooltip_, image_, type_;
		std::vector<std::string> items_;
		int font_size_;
		// below member is valid only context menu
		size_t btnidx_;
		menu *parent_;
		bool button_style_;
	};
	...
	menu* context_menu(const std::string& id);
	void set_main_context_menu(menu* m);
	void set_current_context_menu(menu* m);
	std::vector<menu>& context_menus() { return context_; }
	...
	menu *get_menu_item(const std::string &key);
...
private:
	...
	std::vector<menu> context_;
	menu* main_context_;
	menu* current_context_;
