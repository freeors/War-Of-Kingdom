...
extern void UIKit_wechat(SDL_bool timeline, const char* text, const unsigned char* data, int size);
extern int UIKit_AppInstalled(const unsigned char* app);
extern void UIKit_MixableAudio(SDL_bool mixable);
extern void UIKit_PlaybackAudio(SDL_bool play);

extern void UIKit_ScanPeripherals(SDL_BleCallbacks* callbacks);
extern void UIKit_StopScanPeripherals();
extern SDL_BlePeripheral* UIKit_FindBlePeripheral(const void* cookie);
extern void UIKit_StartAdvertise();
extern void UIKit_ConnectPeripheral(void* cookie);
extern void UIKit_DisconnectPeripheral(void* cookie);
extern void UIKit_GetServices(void* cookie);
extern void UIKit_GetCharacteristics(const void* cookie, const char* service);
extern void UIKit_ReadCharacteristic(const void* cookie, const char* characteristic_uuid);
extern void UIKit_WriteCharacteristic(const void* cookie, const char* characteristic_uuid, const char* data, int size);
extern void UIKit_SetNotify(const void* cookie, const char* characteristic_uuid);
extern void UIKit_DiscoverDescriptors(const void* cookie, const char* characteristic_uuid);
extern int UIKit_AuthorizationStatus();
#else
void UIKit_ScanPeripherals(SDL_BleCallbacks* callbacks)
{
	SDL_BlePeripheral peripheral;

	if (callbacks && callbacks->discover_peripheral) {
		peripheral.name = "M1";
		callbacks->discover_peripheral(&peripheral);
		peripheral.name = "M2";
		callbacks->discover_peripheral(&peripheral);
		peripheral.name = "M3";
		callbacks->discover_peripheral(&peripheral);
...
SDL_BlePeripheral* UIKit_FindBlePeripheral(const void* cookie) { return NULL; }
...
void UIKit_GetCharacteristics(const void* cookie, const char* service) {}
...
int UIKit_AuthorizationStatus() { return 0; }
#endif
...
#endif
		} else if (len == 3 && !SDL_memcmp(name, "app", len)) {
#if defined(__IPHONEOS__) && __IPHONEOS__
			return UIKit_AppInstalled((const unsigned char*)userdata);
#else
			return (void*)1;
#endif
		} else if (len >= 8 && !SDL_memcmp(name, "wechat_", 7)) {
			if (!SDL_memcmp(name + 7, "png", 3)) {
				int data_size = 0;
				const char* text = NULL;
				SDL_bool timeline = name[10] == 't'? SDL_TRUE: SDL_FALSE;
				SDL_memcpy(&data_size, name + 11, 4);
#if defined(__IPHONEOS__) && __IPHONEOS__
				UIKit_wechat(timeline, name + 15, (const unsigned char*)userdata, data_size);
#endif
			}
		} else if (len >= 5 && !SDL_memcmp(name, "ble_", 4)) {
			if (!SDL_memcmp(name + 4, "scan", 4)) {
				UIKit_ScanPeripherals((SDL_BleCallbacks*)userdata);

            } else if (!SDL_memcmp(name + 4, "stopscan", 8)) {
                UIKit_StopScanPeripherals();
                
            } else if (!SDL_memcmp(name + 4, "peripheral", 10)) {
                return (void*)UIKit_FindBlePeripheral(userdata);
                
            } else if (!SDL_memcmp(name + 4, "connect", 7)) {
				UIKit_ConnectPeripheral(userdata);

            } else if (!SDL_memcmp(name + 4, "disconnect", 10)) {
                UIKit_DisconnectPeripheral(userdata);
                
            } else if (!SDL_memcmp(name + 4, "services", 8)) {
				UIKit_GetServices(userdata);

			} else if (!SDL_memcmp(name + 4, "char_", 5)) {
				const char* uuid = name + 9;
				UIKit_GetCharacteristics(userdata, uuid);

			} else if (!SDL_memcmp(name + 4, "read_", 5)) {
				const char* uuid = name + 9;
				UIKit_ReadCharacteristic(userdata, uuid);

            } else if (!SDL_memcmp(name + 4, "write_", 6)) {
                const char* uuid = name + 10;
                int len = uuid[5];
                const char* data = uuid + 6;
                UIKit_WriteCharacteristic(userdata, uuid, data, len);
                
            } else if (!SDL_memcmp(name + 4, "notify_", 7)) {
				const char* uuid = name + 11;
				UIKit_SetNotify(userdata, uuid);
                
            } else if (!SDL_memcmp(name + 4, "descriptors_", 12)) {
				const char* uuid = name + 16;
				UIKit_DiscoverDescriptors(userdata, uuid);

			} else if (!SDL_memcmp(name + 4, "advertise", 9)) {
                UIKit_StartAdvertise();

            } else if (!SDL_memcmp(name + 4, "status", 6)) {
                UIKit_AuthorizationStatus();
            }
