...
BlitRGBtoRGBPixelAlphaMMX(SDL_BlitInfo * info)
{
    ...
    while (height--) {
		...
		Uint32 dalpha = *dstp & amask;
		if (alpha == 0) {
			/* do nothing */
		} else if (alpha == amask || dalpha == SDL_ALPHA_TRANSPARENT) {
			*dstp = *srcp;
		} else {
...
BlitRGBtoRGBPixelAlpha(SDL_BlitInfo * info)
{
    ...
    while (height--) {
		...
		if (alpha) {
			d = *dstp;
			dalpha = d >> 24;
		  if (alpha == SDL_ALPHA_OPAQUE || dalpha == SDL_ALPHA_TRANSPARENT) {
			  *dstp = *srcp;
		  } else {
		    /*
