...
static SDL_bool LoadCAFStream(WAVStream *wave);
...
static SDL_bool LoadCAFStream(WAVStream *wave)
{
	...
	if (format->mFormatID != 0x6d63706c || format->mBytesPerPacket != 2 || format->mChannelsPerFrame != 1 || format->mBitsPerChannel != 16) {
		was_error = 1;
        goto done;
	}
    ...
    chunk.data = NULL;
    do {
        wave->start = (long)SDL_RWtell(src) + sizeof(Uint32) + sizeof(Uint64);
...
{
	int len_mult = wave->cvt.src_format == AUDIO_S16? 2: 1; // only support samplerate: 8000, channels: 1, samplebit: 8/16
